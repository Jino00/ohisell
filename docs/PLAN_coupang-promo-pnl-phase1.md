...
- **`blank_observations`(응답 필드)를 본다.** 키는 있는데 `qty`·`revenue`가 둘 다 빈 값인 행 수다. 이 값이 `ingested`와 같으면 0판매일이 아니라 **매핑 사고**를 의심해야 한다(경고 로그도 함께 나간다). 페처가 `{"qty": row.get("판매수량")}` 꼴이면 컬럼명이 바뀌어도 키는 남으므로 행 단위 검증만으로는 못 잡는다 — 배치로만 보인다.
