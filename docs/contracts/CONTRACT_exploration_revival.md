...
## §11. 완료 QA (별도 Sonnet · 읽기 전용 · 2026-09-05 20:30~20:45 KST) — **원문 그대로**

```
판정(Jino 지시 원문): 부분달성 — 코드·테스트로 「bleeding=42/67 그룹은 표본 부족」 예외가 정확히
  구현·검증됐고(§4-B 전건 달성, 픽스처가 §0 실측 그룹 grp-…59832147/59832150 구조를 그대로 재현),
  prod 배포도 완료(D-NAO-289 3건·effective_bid_getter 5건, 2026-09-05 20:45 KST 확인)됐다. 다만
  "그 광고그룹이 실제로 기회를 되찾는" 라이브 장면은 배포 후 40분간 관측 0건(§4-C ⓘⓛ 판정불능 —
  다음 bleeding 평가 사이클 미도래)이고, 리뷰 이월 P2-8(예외 창 14일 vs 스톱로스 창 3/7일 사이
  미접촉 구간)이 "여전히 아무도 안 만지는 그룹"의 잔여를 인정한다 — 「영원히」 문제의 완전 폐쇄는
  아니다. (2026-09-05 20:45 KST)

판정(계약 D-NAO-289 §4): 부분달성 — §4-A·§4-B 전 항목(S1·S2·ⓐ~ⓖ) 라이브 재현·달성. §4-C ⓗⓚ만
  배포 직후에도 판정 가능해 달성(0건, 구조상 정상/필요). ⓘⓙⓜ·§4-D ⓛ은 계약 스스로 "배포 후 시간
  경과가 필요"라고 못 박은 항목이라 판정불능(미달 아님) — 13개 체크박스 중 즉시 확정 가능한 9개는
  전부 달성, 시간 경과형 4개(ⓘⓙⓜⓛ)는 판정불능. (2026-09-05 20:45 KST)

판정(트랙 D-NAO-59): 판정불능 — 이 슬라이스는 총이익 절대액을 전진시키는 처방이 아니라 탐색 정책의
  오판(표본부족을 손실확정으로 오분류)을 고치는 배선 수정이고, 배포 후 스코프 안 탐색 실쓰기가
  0건(구조상 정상)이라 총이익에 물린 금액을 잴 라이브 원장 자체가 아직 없다. 계약 §9-1도 "M4 합격
  관측에 닿지 않는다"고 스스로 적어 총이익 진전을 주장하지 않는다. (2026-09-05 20:45 KST)

판정(PAO 북극성 M4): 미달 — M4 합격 관측(카나리 optimizer='ours' 1주 연속 실집행+가드레일 위반0+
  되돌림0)과 그 전제인 §8-①②(Jino 결정 2건: optimizer 해제 범위·대행사 소유권 분리) 모두 이
  세션에서 안 움직였다(북극성 §8 표에 두 결정 다 미확정으로 남아 있음, 2026-09-05 20:2x KST 문서
  직접 열람). 계약 §9-1이 스스로 "M4가 전진했다고 쓰지 않는다"고 선언했으므로 이 슬라이스가 M4를
  노린 적도 없다 — 다만 대조축 자체는 미달로 남는다. (2026-09-05 20:45 KST)

종합: 부분달성
```

### ★QA가 잡은 것 — 코디네이터 보고에 없던 사실 (원문)

> **`naver_change_log.changed_at`은 KST로 저장되고 `ops_diary_entries.created_at`은 UTC로 저장된다 — 같은 prod DB 안에서 테이블마다 시간대가 다르다.** 내가 초기에 절대 UTC 문자열로 `naver_change_log`를 필터링했다가 오답(11건)을 얻었고 KST로 재필터링해 정답(1건)을 얻었다.

그 외 코디네이터가 §0·§4-C·트랙에 적은 수치(bleeding 67그룹·42/67·`cost_asof` 4,850행 NULL/0=0·PR #741 병합·CI 3/3 SUCCESS·회귀 7,898/0·순증 +79줄·prod 심볼 0→3/0→5·스코프 6행·pending 기준선 1,164/728)는 **전부 독립 재현으로 일치**했다.

### QA가 확인하지 못한 것 (원문 — 「안 쟀다」와 「재서 0이었다」를 구분)

- **재서 0이었다**: ⓗ 스코프 밖 실쓰기(0/7일) · ⓚ 스코프 안 explore 실쓰기(0) · ⓛ `D-NAO-289` 사유문 등장(0건) · explore diary 배포 후 신규(0행) · prod `shopping_group_bep` `cost_asof` NULL/0(각 0/4,850)
- **안 쟀다(경과시간·창 부족으로 원리적으로 못 잼)**: ⓘ 사유문 이동 24h 비교 · ⓙ §7 대칭 7일 창 · ⓜ 대기카드 7일 증가분(20:45 스냅샷 1건만) · 카나리 M4의 1주 연속 관측
- **코드 열람·테스트로만 확인(prod 라이브 트레이스 미확인)**: `effective_bid_getter` 지연 호출·캐시 1회 재사용 — 아직 그 분기를 탄 사례가 0건이라 트레이스 자체가 없음
- 2R 생존 변이 `p1_reason_values_first`의 재발방지 단언을 **직접 재주입해 죽는지는 재현하지 않음**(해당 테스트가 219 전건 통과 목록에 포함됨만 확인)
- `safe_merge.sh`·`safe_deploy.sh` 스크립트 내부 로직은 감사하지 않고 결과만 재확인

### 미달·판정불능의 처분 (QA는 처분을 지정하지 않는다 — 아래는 코디네이터 기록)

- **ⓘⓙⓜⓛ 판정불능**: 탐색 레인의 bleeding 평가 사이클이 배포 후 미도래. 발화가 관측되면 §8이 허용하는 **재판정 1회**로 확정한다.
- **북극성 M4 미달**: 이 계약이 노린 적 없는 대조축이고(§9-1), 해소는 §8-①② Jino 결정 2건에 달려 있다.
- **Jino 지시 원문 부분달성의 잔여**: 리뷰 이월 P2-8(예외 창 14일 vs 스톱로스 창 3/7일 사이 구간)은 **별건 계약** 감이다.
