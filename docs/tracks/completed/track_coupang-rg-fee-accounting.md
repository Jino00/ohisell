...
### ★2026-08-03 후속 세션 (PR #190 + S9) — 감사 정확도 마감
- **S9 완료(코드·배포)**: 감사가 추론하던 두 값을 **정산서 기재값으로 교체**. `CoupangRgSettlementFee`에 `billed_size_type`(개별포장사이즈=청구에 쓴 등급)·`billed_order_count`(distinct 주문ID)·`billed_quantity`(Σ판매수량) 추가(alembic `rg9billed7c4e`, prod 적용). 근거: 정산 엑셀 상세에 세 값이 원래부터 있다(ref 17 §8-1). 감사는 정산서 1순위 → 물류센터 실측 → 등록치수 순으로 폴백하고, 플래그 이름이 근거의 세기를 말한다(`billed_size_vs_amount_mismatch` > `measured_vs_billed_mismatch` > `size_mismatch_high`). 회귀 11건, 4,448 passed. **읽기 전용·net_profit 불변(D-17)**.
- **⚠️ S9 컬럼 백필 미완(잔여 작업 1)**: 새 컬럼이 아직 전부 NULL이다. Mac 페처는 **"prod에 옵션 행이 없는 주기"만** 내려받으므로(`layer2-gaps`), 행은 있고 컬럼만 NULL인 상태를 결손으로 보지 않는다(라이브 확인: `열거 6개 / prod 결손 1` → WAREHOUSING_SHIPPING 미수집). 선택지 = ⓐprod Wing 쿠키 갱신 후 `POST /rg/settlement/auto-download` 전 기간 재수집(즉시·Jino 동의 필요, 쿠키를 옮기는 일) / ⓑ방치(매주 새 주기부터 자동 충전, 4·5월 옵션은 영구 NULL) / ⓒ결손 판정에 "컬럼 NULL" 추가(구형식 파일이 영구 결손 → **무한 재수집 위험**, 비추천).
- **감사 뷰 프론트 완료**: 재고관리 > 로켓그로스 > 서브탭 `청구 감사`(`RgFeeAuditView.tsx`). 신호 세기 순 배지 + 청구/실측 사이즈 나란히 + `divisor_source`(단가 신뢰도) + `표본(판정/전체)` + 주기별 드릴다운(`청구총액` vs `판정액` 분리) + disclaimer 노출. 라이브 렌더 확인. **프론트에서 단가를 재계산하지 않는다**(`charged ÷ order_count`가 오탐 4건의 원인이었다).
- **CATEGORY_TR 매핑 완료(활성화는 보류)**: 시트명 `주문내역, 판매수수료` → `sale_fee`. 단 페처 config `rg_report_types`에 **넣지 않았다** — 실파일 컬럼 구조 미확인 상태로 켜면 파서가 못 읽어 `strict_sheets` 경로가 422 → 재시도 소진(코드 자체 경고). **잔여 작업 2**: 실파일 1건 확인 후 활성화.
- **★below_floor 규명 = 과소청구(우리 손해 아님)**: `95501699184`(오픽스 맥세이프 카드지갑) 주문당 176.8원 = **쿠팡이 6주 연속(06-08~07-19) 배송·입출고비를 0원으로 청구**했다. 청구가 발생한 주기의 단가는 정상(17,775=1,975×9 등). 주문 156건 중 과금된 건 약 14건뿐(미청구 ≈28만원). 음수 환급·저가할인(판매가 16,900원)·합포장 분산은 전부 기각. **왜 0으로 산정됐는지는 DB로 확인 안 됨** → Wing 정산 상세 수동 대조 필요(**잔여 작업 3**). 나머지 5건도 같은 계열(주기 결측), `95577773674`만 음수 환급.
- **잔존 오탐 1건 화면에서 자기설명됨**: `95570603539` 2,850원(2.11배)은 주기 드릴다운에 `07-01~07-05 = 3,800원 / 1주문`으로 보인다(3,800=2×1,900인데 주문 1건 수집). 표본 하한 도입은 저볼륨 옵션의 진짜 신호를 죽이므로 미도입 — S9 백필이 들어오면 자동 해소된다.
- **정정 기록**: 이 세션 중 "6월 말~7월 주문 수집 결손"이라고 한 판단은 **틀렸다**. 단가를 1,800원으로 잘못 잡은 결과였고, 실제 단가 2,100원으로 계산하면 `75,600÷2,100=36건 = DB 주문 36건`으로 정확히 일치한다. 6월 이후 주문 결손은 없고, 주기별 노이즈의 원인은 **매출인식일 ≠ 결제일**이다.
- **S8 후속(선택)**: ★2026-06-15 라이브 재감사 — size_mismatch_high는 이제 **1건**(이전 4건은 PRODUCT_SIZE_COMPARISON 실측 사이즈 자동수집되며 해소). 남은 1건 = 아이패드미니필름(91313543029, 등록 극소형 세변합 60.5cm vs 배송청구 주문당 4,050원=대형1 정합 3배, size_source=registered_dims 실측 미확보). **Jino 결정(2026-06-15): 자동해제 대기** — 다음 입고 시 실측 사이즈 수집되면 자동 판가름(ⓐ극소형 확정→과오청구 / ⓑ대형1 확정→정당), 코드변경 없음. 프론트 UI(로켓그로스 탭에 감사 뷰) 추가는 미정.
  - **★2026-08-03 판정 완료 = 과오청구 아님(ⓐⓑ 어느 쪽도 아니었다).** 정산 원본 대조 결과 **주문당 실청구 배송비는 2,025원**(4,050원이 아니다). 이 옵션의 옵션단위 정산 행은 주기 4개(04-13~19·04-20~26·05-11~17·05-18~24)이고 **주기마다 정확히 delivery 2,025 / warehousing 1,175**, 주문 데이터가 있는 2주기는 각 1주문·1개 → 주문당 단가가 직접 확인된다. 4,050은 감사가 **주기 4개 합계 8,100을 주문 2건으로 나눈 값** — 분자(정산 수집 2026-03-16~)와 분모(RG 주문 수집 2026-06-03 시작·백필 최대 2026-05-05) **창이 달라서 생긴 산술 산물**이다. 2,025원은 대형1 최소금액 2,200원에도 못 미치므로 청구액 자체가 대형1일 수 없고, 계정 전체 극소형(실측) 옵션의 주별 원시 배송비 분포도 1,975(71주기)·1,900(37)·2,025(5)·2,100과 그 정수배뿐 — 4,050 단가는 존재하지 않는다.
  - **같은 원인의 오탐 4건 전부**: `measured_vs_billed_mismatch` WING2 1건(91313543029) + WING1 3건(94156664759·72·81, 오픽스 필름 — 주기 2개×1,900=3,800을 주문 1건으로 나눔). 주문 수집 창(2026-05-11~)으로 조회하면 **4건 모두 플래그 0**(라이브 확인). → **S8 감사 결함**: per-unit 정규화가 분자·분모의 커버리지 일치를 확인하지 않는다. 읽기 전용·net_profit 무영향(D-17).
  - **★수정 완료(2026-08-03, PR #190, prod 배포)** — Jino 지시 "ⓒ+ⓑ로 수정해서 한 PR로". 단 **ⓒ(주기 단위 판정)는 라이브가 반증해 집계 판정으로 정정**했다:
    - ⓑ **미대응 주기는 분모에서 제외**하고 `periods_total/judged/unmatched`로 표면화. 대응 주기 0개면 `unit_unknown` 강등(판정 보류). 조회범위는 주기를 고르고 주기가 주문을 고른다 — 주문을 범위로 다시 자르지 않는다(경계 재발 차단).
    - ⓒ→**집계 판정으로 정정**: 주기별 판정을 배포해 재조회하니 목표 4건은 사라졌으나 WING1 `measured_vs_billed_mismatch`가 **3→20**, `below_floor` 6→31(flagged 13→44/82)로 늘었다. 원인 = 커버리지 결손이 0주문 주기에만 있는 게 아니라 **주기 안에도** 있다(95570603482가 `1,975/1주문`과 `3,950/1주문`을 함께, 95598292078이 `12,600/7주문`과 `12,600/3주문`을 함께 가짐 — 금액은 안정 단가의 정수배인데 분모가 불안정). 주기별 판정은 표본을 주기 1개로 줄여 그 불안정을 **증폭**한다. → **판정 = 판정주기 전체 합산 1회**, 주기별 값은 `period_detail`·`periods_flagged`·`summary.clean_but_period_outlier`로 표시만.
    - **라이브 최종(2026-08-03)**: WING1 `measured_vs_billed_mismatch` 3→**1**·flagged 13→12·`unit_unknown` 4→6(강등)·`clean_but_period_outlier` **32**(주기별 판정이었다면 82옵션 중 32개가 플래그였다는 증거). WING2 1→**0**·flagged 6→5. 4개 대상 옵션 = 3건 무플래그(단가 2,025·1,900·1,900) + 1건(94156664772) 판정보류.
  - **잔존 오탐 1건(명시)**: `95570603539` — 주기 2개(1,900/1주문 + 3,800/1주문) 합산 5,700÷2=2,850원 = 극소형 최소의 **2.11배**로 임계 2.0을 간신히 넘겼다. 3,800=2×1,900이라 그 주기도 2주문분 청구·1건만 수집일 가능성이 높지만 **청구된 주문수를 알 방법이 없다**(금액÷알려진단가 역산 = 쿠팡 계산기 복제 = D-17 금지). 표본 하한(예: 판정 주문 3건 미만은 보류) 도입은 저볼륨 옵션의 진짜 신호도 함께 죽이므로 **Jino 결정 사항으로 남긴다**.
  - **별건 미규명**: `below_floor` 6건 중 95501699184은 정산·주문 창이 일치(06-08~08-02)하는데도 주문 154건에 배송비 27,575원(주문당 179원) — 커버리지 문제가 아니다. 원인 미규명(정산 상세 행 부분수집 또는 프로모션 환급 가능성, 단정 금지).
