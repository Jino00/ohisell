...
### D-17. 아이템별 BEP RoAS 정식 기능 (B안) — 설계 결정 (2026-06-16 Jino 승인)
- **배경**: 직전 세션에서 옵션 1건(95521944483) BEP RoAS 수작업 산출(공급가 215%/현금 186%). Jino가 정식 기능(B안) 선택 → 옵션별 BEP RoAS 자동 산출, 종합조망 광고축에 actual RoAS와 나란히 표시. 계산 정본 = 메모리 `bep-roas-calculation-structure.md`(BEP = 판매가 ÷ 공헌이익, 공헌이익=판매가−판매수수료−풀필먼트/운송비−원가).
- **D-17a 참조표 범위 = 우리 카탈로그 우선**: `coupang_commission_rate`(카테고리→요율)·`coupang_rg_fulfillment_schedule`(카테고리군×사이즈6×판매가구간→정가/할인가, 입출고/배송 split) 두 참조표를 **오하이가 실제 파는 카테고리만 먼저** 디지털화(보호필름·케이스·카드지갑·골프필름·도어락필름·그립톡 등). 미커버 카테고리 옵션 = BEP "미산출"(D-3 사실만, 점진 확대). Jino 원문: "우리 카탈로그 우선으로 시작하자".
- **D-17b 표시 위치 = 광고축 옵션 테이블 컬럼**: 별도 패널 아님. actual RoAS 옆에 BEP 컬럼 추가(같은 줄에서 손익분기 대조).
- **D-17c 채널 = 2P·3P 둘 다 항상 표시**: 옵션이 실제 팔린 채널만이 아니라 2P(로켓그로스 풀필먼트)·3P(한진 운송비) BEP를 **둘 다** 표시(채널 전환 시뮬레이션 겸용). Jino 선택 "(b)".
- **D-17d 할인가/정가 = 할인가 메인 + 정가 보조(툴팁)**: 2P 풀필먼트만 해당(3P 한진 1,900 정액). 메인 BEP = 현재 실제 과금되는 **할인가** 기준(원칙22, 지금 실제 빠지는 비용). 정가 기준 BEP = 툴팁 보조(프로모션 종료 대비). Jino 원문: "그러자".
- **D-17e 풀필먼트 드리프트 감지(stale 방지) — Jino 승인 핵심**: 손으로 적은 물류비표(할인가 3,100)는 프로모션 종료 시 stale → 정적 표만으론 못 잡음(Jino 지적). **표(기준값) vs RG 정산 실측(이미 옵션별 수집중) 비교 → 추세 벌어지면 깃발 + Jino 보고**(D-11/D-13 안전 패턴 재사용, 시스템 자동판단 금지). ⚠️주의: 정산 실측은 합포장·recognition 타이밍 때문에 표값과 정확히 안 맞음 → "위로 올라가는 추세" 감지 방식. Jino 원문: "3100원에서 3850원으로 올라갔을때 빠르게 캐치할수 있어?" → 드리프트 감지로 합의("그러자").
- **D-17f 원가 미등록 옵션 = 솔직하게 비움(미산출)**: 원가 없으면 BEP "원가 등록 필요"로 비움(cost=0 가정 금지=거짓신호, 추정 금지). 종합조망 기존 amber "원가 미설정" 패턴 일관. **Jino가 원가 직접 채워넣음**(나는 비어있는 원가 목록 제공). Jino 원문: "어, 비워야 하고, 내가 채워넣도록 할께. 비워져있는 원가를 알려줘".
- **레고 구조(초안, 승인 전)**: Harness `bep_roas_harness`(SA 출력 유통 허브) + SA `commission_rate_resolver`(2P=3P)·`rg_fulfillment_resolver`(2P 정가/할인가)·`seller_shipping_resolver`(3P 한진 재사용)·`cost_resolver`(product_master)·`bep_roas_calculator`(순수함수 현금·공급가). → 다음: 전체 구조 도표 Jino 승인 → Opus 계획서 → Sonnet 구현 → codex.

### D-18. "수수료" = 판매유형별 쿠팡 총비용 — 이익 정의 확정 (2026-06-19 Jino)
- **이익 정의(Jino 원문)**: "이익은 판매가−원가−수수료(쿠팡이 가져가는 모든 비용)−광고비를 했을때의 비용이지. 그러니까 수수료라고 하면 1P, 2P, 3P 판매 경우에 모두 다르지." + "우리가 쿠팡 내의 수수료율을 모두 확인해서 수수료율 계산을 했었잖아."
- **핵심**: 패널/조망의 "수수료"는 판매수수료 하나가 아니라 **쿠팡이 가져가는 모든 비용**이며 판매유형별로 구조가 다르다.
  - **3P (마켓플레이스/Wing)**: 판매수수료(카테고리율, 실측 ~7.8%) + VAT. (한진 배송비는 택배사 비용=수수료 아님, 원가도 아님 → 이익에서 별도 차감)
  - **2P (로켓그로스)**: 판매수수료+VAT **+ 풀필먼트(입출고·배송·보관) + RG정산 광고** (≈ RG 수수료 회계 트랙 D-16 `rg_total`). 매출의 ~19.5%+.
  - **1P (로켓배송/직매입)**: 판매수수료 개념 없음 — 매출=쿠팡 공급가(매입가). 이익=매출−원가−광고 (로켓1P 트랙 D-1~8). ✅ Jino 확인("1P는 너가 이야기한게 맞아", 2026-06-19).
- **현 상태(버그/미구현)**: 쿠팡 운영 패널 `sales-summary`는 `fee_rate_map`이 빈 dict라 **전 옵션 flat 7.8%(3P 판매수수료만)** 적용 → 2P 보관료·RG광고 누락으로 **2P 이익 과대평가**. 등록율 `sale_agent_commission`은 prod 전부 0/NULL(죽음), 실측율 `service_fee_ratio`는 정산된 옵션만(오늘 신규 6옵션은 이력 0).
- **근본**: 판매유형별 쿠팡 비용 모델(`coupang_commission_rate`·`coupang_rg_fulfillment_schedule` + commission/fulfillment resolver)은 **D-17에서 설계·승인됐으나 코드 미구현**(트랙 "초안, 승인 전" 그대로). 이 미구현이 flat 7.8%의 원인. 2P 전액은 RG 수수료 회계 D-16에 있으나 **종합조망 net_profit에만** 반영(쿠팡 운영 패널엔 미반영 = 회계엔진 2개 평행).
- **다음(원칙4/18, 코딩 전)**: 구조 설계(판매유형별 쿠팡비용 SA 재사용/신설 도표) → Jino 승인 → Opus 계획서 → 구현 → codex. D-17 BEP 자원과 공유(commission/fulfillment resolver는 BEP·이익 양쪽이 소비).
- **D-18a 구조 확정 (2026-06-19 Jino "시작하자")**: Agent(sales-summary 패널) → Harness `coupang_seller_cost`(판매유형별 쿠팡 총비용 유통 허브) → SA 3개 `commission_resolver`(판매수수료+VAT, 2P=3P)·`rg_fulfillment_resolver`(2P 입출고·배송·보관)·`rg_ad_resolver`(2P RG광고). 각 SA: 실측(정산) ▸ 옵션 실측율/단가 ▸ 추정 캐스케이드. 범위=주문기반 **2P+3P**, 1P은 로켓1P RocketView가 별도(매출=공급가·수수료0).
- **D-18b 한진 배송비(3P) = 별도 '물류비' 차감 (2026-06-19 Jino 선택)**: 한진 1,900/건은 쿠팡 수수료 아님(택배사) → 수수료 카드에 미포함, 이익에서 별도 물류비로 차감. **이익 = 매출 − 원가 − 쿠팡수수료 − 마켓플레이스광고 − 물류비(한진 3P)**.
- **D-18c 미정산 2P 보관료·RG광고 = 추정하되 표기 (2026-06-19 Jino 선택, 원칙22)**: 정산 전이면 보관료=카테고리/실측 단가 추정·RG광고=최근 실측비율 추정, basis=estimate로 명시 표기(은폐 금지). 계획서=`docs/PLAN_coupang-seller-cost-by-saletype.md`.
- **D-18 구현 완료 (2026-06-19 커밋 dff0692+e189d75, prod 배포·self-verify 완료)**:
  - S1: `commission_vat_resolver.py` (순수함수, fixture 10/10) — 3P actual▸optionRate▸default, 2P rg_settled▸rg_estimated, VAT×1.1 1회
  - S2: `rg_cost_reader.py` (순수함수+DB reader, fixture 10/10) — load_settled_option_costs/load_account_storage/load_rg_ad_rate + compute_2p_cost
  - S3: `coupang_seller_cost.py` Harness — compute_batch(배치주입 N+1방지)
  - S4: `coupang_ops.py` 컷오버 — fee=판매유형별쿠팡총비용(fee_legacy/fee_new 병행출력 후 컷오버), RG shipping 제거(풀필먼트=fee에 포함), legacy 삭제
  - S5: `CoupangOps.tsx` "수수료"→"쿠팡 비용", shipSub=한진전용 설명
  - **prod self-verify(원칙22)**: Wing fee=13,299(≈8.58%), RG fee=158,886(풀필먼트포함)·shipping=0 ✓. codex review 대기(원칙19 게이트).

## 3. 사용자 원문 인용 (왜곡 방지)
- "종합적으로 오픽스의 판매현황에 대한 조망을 하고 싶어. 회계뿐 아니라 광고 전략, 상품판매 전략등까지 말이야"
- "이익은 판매가−원가−수수료(쿠팡이 가져가는 모든 비용)−광고비를 했을때의 비용이지. 그러니까 수수료라고 하면 1P, 2P, 3P 판매 경우에 모두 다르지." (2026-06-19, 이익/수수료 정의 D-18)
