...
### D-15. 쿠팡 API 전수 디테일 수집 — 100% 레퍼런스 확보 (2026-06-03 Jino 지시)
...
- **결정**: 쿠팡 API **두 표면 모두 전수 디테일 수집** → references 카탈로그.
  - ① **공식 API 문서**(developers.coupangcorp.com, 공개·로그인 불필요): 11섹션 100엔드포인트 full detail(path·method·params·req/resp schema). 기수집=상품(02)·반품/교환(03)·정산(04)·로켓그로스(05). 미수집=쿠폰/캐시백21·배송환불12·물류8·카테고리6·CS6·브랜드3 + 상품 잔여.
  - ② **Wing 포털 내부 API**(wing.coupang.com/tenants/*, 비공식·세션쿠키·로그인 필요): 포털 프론트가 쓰는 내부 엔드포인트(입고 등 공식문서에 없는 것). 네트워크 캡처로 매핑. ⚠️비공식이라 연동 시 안정성 트레이드오프(D-14) — 매핑은 하되 사용은 건별 판단.
  - 11섹션 URL: 상품 360005046534·쿠폰/캐시백 360005046574·카테고리 360005046514·교환 360005046554·배송/환불 360005081913·물류센터 360005081873·반품 360005081933·정산 360005081973·CS 360005081953·브랜드 58348875092889·로켓그로스 35157469062553.
- **목적**: Claude가 추정으로 "없다" 하지 않도록 권위 레퍼런스 확보(원칙22·추정금지). 트랙 D-1(100개 전부 연결)의 토대.
