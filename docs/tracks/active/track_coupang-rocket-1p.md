...
> 생성: 2026-06-15 · 상태: 🟢 Active (4/6, S4 완료 + S4.5a 발주상세 수집 + S4.5b 원가 브리지 매핑 완료) · 계정: 주식회사 오하이테크
...
- [ ] S5 프론트: 종합조망 로켓배송 뷰/축 + 갱신 버튼 + **원가 매핑 관리 UI**(미매핑 목록·제안 클릭 확정)
- [ ] S6 prod 라이브 self-verify + codex + 배포
(스프린트 수는 S1 정찰 결과로 확정)

## 현재 진행 단계
- **S4.5b 완료(2026-06-18)**: 원가 브리지 매핑 — 모델 `RocketProductCostMap`(product_number→internal_sku, status confirmed|ignored) + alembic `r2s3t4u5v6w7`(head) + 순수 SA `suggest_skus`(difflib) + Harness `rocket_cost_map.py`(list_unmapped/list_mappings/upsert/delete) + 라우터 4종(사용자 CRUD). net_profit 불변(매핑만, 원가 결합=S4.5c). 테스트 11+전체 **309 통과**. 실 DB e2e(894 마스터·실 제안·confirm/ignored/delete) 검증·오염 cleanup 원복. **다음 S4.5c**: `rocket_intelligence`에 `_rocket_cost` SA(Σ order_qty×cost_price[매핑] 발주일 윈도우) → net_profit cost 반영(has_cost=true, D-12 해소)+커버리지%. ⚠codex·prod 배포는 6/19 quota 리셋 후 S2+S3+S4+S4.5a와 묶음.
...
## 다음 액션
1. **S4.5c 원가 결합(다음 스프린트)**: `rocket_intelligence`에 `_rocket_cost` SA — Σ(po_item.order_qty × cost_price[`RocketProductCostMap` 매핑된 internal_sku]) 발주일 윈도우, net_profit cost 반영(has_cost=true 전환, D-12 잔여 해소) + 커버리지%(매핑된 발주금액 ÷ 전체 발주금액, 미매핑 SKU 투명화). ignored는 원가 0 처리하되 커버리지 분모엔 포함.
2. **(quota 리셋 후 6/19) `/codex review`** — **S2+S3+S4+S4.5a+S4.5b** diff 교차검증(원칙19). pass면 ① prod 배포(scp 모델/라우터/services/마이그레이션 + `alembic upgrade head`[q1r2s3t4u5v6, r2s3t4u5v6w7] + `pm2 restart ohisell-backend`) ② launchd 설치(`cp tools/com.ohisell.rocket.plist ~/Library/LaunchAgents/` + load) ③ prod 라이브 self-verify(페처 run→prod 세 테이블 적재 + `GET /api/overview/rocket-overview` + `GET /rocket/cost-map/unmapped` 확인) ④ git push. fail이면 대화형 반영.
3. **S5 프론트(D-10)**: 돈축=종합조망 1P(`rocket-overview` 소비) + **원가 매핑 관리 UI**(미매핑 목록·제안 클릭 확정·ignored) / 운영축=재고·발송 관제(발주→입고 진행) + 온디맨드 '갱신' 버튼. S6 prod self-verify+codex+배포.
