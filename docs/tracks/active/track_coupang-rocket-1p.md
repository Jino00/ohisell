...
- **M1 유지보수 완료(2026-07-28, 미푸시)**: 파서 누락 컬럼 2건 복원(체크리스트 M1 참조). 커밋 `85967cf`→`e3da1f6`→`39c1c39`, alembic `f6a8c0b2d4e6` **단일 head**, 루트 전체 3505 passed.
  ...
  - ⚠**배포 순서 강제**: `alembic upgrade head` → 코드 순. ingest 경로가 엔티티를 통째 SELECT하므로 컬럼 없는 DB에 `models.py`만 올라가면 신규 필드가 아니라 **정산·발주상세 ingest 전체가 OperationalError로 침묵**한다. `scripts/safe_deploy.sh`에 alembic 단계가 **없어서**(grep 실측 0건) 자동 보장되지 않는다 — PR 본문에 3단계를 복사할 것.
