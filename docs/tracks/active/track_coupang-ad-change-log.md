...
### D-CAC-6 변경 주체 = Jino 단일 (2026-08-04)
Jino 원문: **"물론 쿠팡은 대행사가 없으니까 모두 내가 변경주체야"**

네이버에선 주체(우리 자동화/대행사/Jino)를 가르는 게 핵심 축이었지만 쿠팡은 **상수**다.
화면이 주체를 표시하지 않는 현재 설계가 옳다 — 공백이 아니라 자명해서 안 쓰는 것이다.
(단 문구는 "Jino 또는 대행사"로 돼 있다 → 다음 손볼 때 "Jino"로 좁힐 수 있다.)

### D-CAC-7 미확인 축은 Jino가 운영 시점에 알려준다 (2026-08-04)
Jino 원문: **"너가 확인 못한 부분은 내가 해당 이벤트나 목적광고를 운영할때 별도로 말을 해줄께"**

대상: ①「인지도 상승」 등 새 `goalType` ②`BUDGET`/`TROAS`/`CAMPAIGN_ONOFF`/`VIID` 밖의 changeType
③디스플레이 광고 개시.
★**추측으로 미리 넣지 않는다** — 2026-08-04에 추측·부실검증으로 두 번 틀렸다(§아래 교훈).

### 중간값 뭉침은 결함이 아니라 성질 (2026-08-04, Jino 확인)
Jino: "중간값이 왜 필요한지 모르겠네?" → 맞다. "안 잡히는 것"에 넣었던 건 과했다.
- 뜻: 갱신 두 회차 **사이에 같은 항목을 두 번 이상 저장하면** 스냅샷 축은 순증분만 보인다
  (입찰가 100→200→150이 "100→150"). **계정 구분과는 무관하다.**
- ★**저장한 것만 해당된다.** 입력창에서 썼다 지웠다 하는 건 변경이 아니다 — 쿠팡도 기록하지
  않고 우리도 모른다(Jino 확인: "혼자서 여러번 썻다 지웠다 하는걸 말하는건가? 그건 상관없지 않나?").
- 실제로 손실이 되는 유일한 경우는 **며칠에 걸쳐 같은 항목을 두 번 저장하고 그 사이 갱신을
  한 번도 안 누른 때**다. 한 자리에서 연달아 바꾼 건 사실상 한 번의 결정이라 순증분이 맞다.
- ★쿠팡 이벤트 축(예산·ROAS·On/Off·소재개수)은 **안 뭉친다** — 실측: 「메츨 중」 소재 제거가
  13:16:49 · 13:18:12 · 13:18:35 세 줄로 각각 남았다.
- 뭉칠 수 있는 건 광고그룹 15종·캠페인 설정 13종·소재 옵션ID뿐.

## ★교훈 — 오늘 두 번 틀렸다

1. **"비-PA 10.6%는 디스플레이 광고일 것"** → 아니었다. 디스플레이는 집행 0건.
   정체는 **NCA(신규 구매 고객 확보)** 캠페인이었고, Jino가 광고센터 화면을 보여줘서 밝혀졌다.
2. **"`goalType` 필터는 아무것도 안 숨긴다"** → `goalType: null`로 바꿔보고 16=16이니
   안심했는데, **null은 필터 해제가 아니라 기본값(SALES)**이었다. 값을 **실제로 훑었어야** 했다.
   그 탓에 오픽스 4건·오하이테크 48건이 통째로 빠져 있었다.

공통 원인: **"바꿔봤는데 결과가 같더라"를 "영향이 없다"로 읽었다.** 음성 결과가 나올 때
그 실험이 실제로 무엇을 시험했는지 되묻지 않았다.
