...
## 확정 결정사항 (D-N)
- **D-19 (발송관제 화면 이전 → 재고 관리 페이지, 확정 2026-06-18)**: RG 발송관제를 `/coupang-ops`(로켓그로스 탭)에서 떼어 **`/inventory`(재고 관리) 페이지**로 이전한다. 사유(Jino): 발송관제는 매출/수익성이 아니라 **재고/물류 이슈**라 성격이 섞임. 재고 관리 페이지는 **멀티채널 재고 관제 허브**로 설계 — 채널 탭 [로켓그로스(RG) | 로켓배송 1P | Wing], RG만 우선 구현·1P/Wing은 향후 입주(Jino "나중에는 로켓배송 1P·Wing 재고까지 모두 들어와야지"). 화면은 **모든 RG 상품 노출(showAll 기본)** + 컬럼 정렬·상품명 검색. **백엔드·API·머니룰 무변경**(build_replenishment_plan이 이미 전체 옵션 반환). 프론트 전용 리팩토링. 계획서=`docs/PLAN_inventory-page-replenishment-move.md`. 원문: "RG발송관제를 별도로 빼자. 이건 재고 관련 이슈이니까. 빼서, 모든 상품명을 노출시키고, 거기서 sorting해서 사용할 수 있도록".
- **D-20 (in-transit freshness 게이트 status 인지 — 버그픽스, 2026-06-19 Jino "확인해봐")**: `in_transit_estimator._fetch_freshness`가 쿠키 status를 무시하고 `last_success_at` 나이(2일)만으로 fresh 판정하던 버그. WING1 쿠키 만료(red, last_success 6-17 16:54) 후 sync가 멈춰 입고(1070255256631250944, vii 95536607339)의 판매개시(6-18 08:36) 전환을 못 잡았는데, 마지막 성공이 2일 이내라 stale 입고가 fresh로 통과 → Open API 현재고(`orderable_qty=91`, synced 6-18 20:40)는 이미 흡수한 100을 발송중으로 재계상해 **유효재고 191(유령 +100)**. 수정: fresh는 **status=='green' 계정만 인정**(red/unknown=in-transit 차감 스킵 → effective_stock=현재고만, 안전방향 D-12 품절회피). 표시용 `last_fetch_at`은 status 무관 최근 성공 노출. 회귀 4건(`test_in_transit_estimator` 16/16). **prod 라이브 검증: effective_stock 191→91·in_transit_fresh true→false** (원칙22 — 라이브 권위[쿠키 red] 무시한 fresh 단정 교훈). 커밋 `cfe06e7`. ★운영 후속: Wing 쿠키 갱신(`POST /api/coupang/ops/inbound/cookie`) 시 status green→in-transit 정상 재개+판매개시 stowing_at 포착.
