...
- **D-17 원가 매핑 5건 확정 — 순이익이 그만큼 내려간 것이 정상이다 (2026-08-03, Jino 지시 "원가 매핑 5건 확정해줘")**: D-16이 드러낸 순이익 미도달 상위 5건을 `rocket_product_cost_map`에 확정했다. **이름 유사도로 고르지 않았다** — 기존 확정 매핑이 만든 규칙을 따랐다(이 시스템의 매핑은 이름이 아니라 제품군·원가 판단으로 정해져 있어, 이름 유사도로 고르면 틀린다).
  ...
  - **★D-17 정정(2026-08-03 22:50, Jino 지적 "잘못된 것도 있는데?")**
    - **도출 근거가 무너졌다.** D-17에서 "기존 확정 매핑이 만든 규칙을 따랐다"고 했으나, 그 선례들(18371042·62922000·62921998·62178970·39017749)은 전부 `match_method='suggested'` + `note='auto score=0.56~0.78'`로, **2026-06-17 23:25~23:31에 이름 유사도로 일괄 자동 확정된 것**이었다. 즉 자동 매칭 결과를 "사람이 정한 규칙"으로 오독하고 그 위에 5건을 쌓았다 — "이름 유사도로 고르지 않았다"는 D-17의 서술은 **사실과 반대**였다.
    - **되돌린 3건**: `18371038`(아이폰13/13프로 → 아이폰17 Pro 마스터, 모델 4세대 차이) · `76350897`·`76350898`(지문방지 **무광택** → **사생활보호** 마스터 3,890. 근거였던 폴드7 매핑은 auto score 0.56이고, 무광 계열 마스터는 따로 38개·단가대 1,691원).
    - **정정한 1건**: `39017747` OHI-0298(2,516) → **`OHI-TGLASS-IP17PRO`(3,400)**. ★Jino 확정: "39017747 이것도 원가가 3400원이야". 발주명의 **"이지솔루션"=EZ툴**이므로 제품군은 풀커버 강화유리+EZ툴(3,400)이지 유리코팅 고화질(2,516)이 아니다.
    - **유지 1건**: `62178971` → `OHI-TGLASS-IP17PRO`(3,400) — 같은 제품군, 원가 일치.
    - **최종 상태**: 매핑 2건(39017747·62178971) 유효, 3건 미매핑. 계정 net_profit **37,635,542**, SKU 순이익 도달률 **83.84%**.
    - 계정 순이익 32,805,918 → 37,727,478(+4,921,560, 삭제 원가와 원 단위 일치) → 정정 1건 반영 후 37,635,542.
    - **파생 발견(감사 대상)**: 형제 `39017749`·`39017751`·`39017754`가 auto 0.65로 `OHI-0295/0296/0297`(2,516)에 붙어 있는데, Jino가 확정한 계열 원가는 3,400이다 → **이 3건은 지금 틀린 상태로 남아 있다**(183건 감사에서 처리하기로 Jino 결정).

- **D-18 1P 원가의 96%가 이름 유사도 자동 매칭 위에 있다 — 감사를 별도 작업으로 (2026-08-03, Jino 결정 "다음 작업으로 잡기")**: `rocket_product_cost_map` confirmed 189건 중 **183건이 auto-suggested**(2026-06-17 23:25~23:31, 6분). 이 원가는 표시용이 아니라 `_rocket_cost` → 계정 `net_profit`에 직접 들어간다. 30일 원가 13,701,977원의 출처 분해:

  | 출처 | 건수 | 30일 원가 | 비중 |
  |---|---|---|---|
  | manual(사람) | 3 | 547,733 | 4.0% |
  | auto 0.9+ | 8 | 629,520 | 4.6% |
  | auto 0.8~0.9 | 49 | 1,325,463 | 9.7% |
  | auto 0.7~0.8 | 44 | 6,735,984 | 49.2% |
  | auto 0.6~0.7 | 73 | 3,686,719 | 26.9% |
  | auto 0.6 미만 | 9 | 776,558 | 5.7% |

  **score 0.8 미만이 원가의 81.7%.** 확인된 오류 실물: 0.76이 아이폰13을 아이폰17 Pro에, 0.56이 무광택을 사생활보호에 붙였다. → 감사는 별도 작업으로 예약(세션 chip `task_94245b1d`). 우선순위는 score가 아니라 **30일 원가 금액 × 불확실도**. 구조적 원인은 **화면·API가 사람 확정과 자동 매칭을 구분해 보여주지 않는 것**이므로 그 표면화가 감사의 1번 산출물이다.

- **D-19 1P 원가는 sellc 등록원가가 정본 — 이름 유사도 자동매핑은 폴백이고 출처를 남긴다 (2026-08-03, Jino 지시 "원가는 sellc에 있는 원가를 사용하자")**

  - **왜**: 두 원가의 신뢰 등급이 다르다. `product_channel_mapping`은 쿠팡이 주는 `externalVendorSku`가 `product_master.internal_sku`와 **정확히 일치할 때만** 붙는다(`product_sync._link`) — 코드 대 코드 일치라 사실이다. 반면 `rocket_product_cost_map`의 183/189건은 이름 유사도 자동 확정(D-18)이다.
  - **우선순위**: ①sellc 등록원가 ②rocket 자동매핑(폴백) ③ignored=0 ④미상. ignored 22건도 같은 6분 배치(created_at 06-17 23:25~23:31)라 사람의 제외 결정이라는 보장이 없어 sellc보다 뒤에 둔다. 진짜 사람 결정은 `69411570` 하나뿐.
  - **계정 net_profit과 SKU 손익표가 같은 해석기**(`_resolve_unit_cost`)를 쓴다 — 원가축이 둘로 갈리면 어느 쪽이 맞는지 아무도 모른다.
  - **출처를 행마다·커버리지마다 남긴다**(`cost_source`, `cost_by_source`, `ad_cost_sellc/auto`). 신뢰 등급이 다른 값을 같은 이름으로 저장해 구분을 잃은 것이 이번 사고의 구조적 원인이다.
  - 경로: 발주는 상품번호 그레인, sellc 매핑은 옵션ID 그레인 → **브리지(D-16 `coupang_rocket_option_sku`)가 필수**다(브리지 sku가 채널매핑에 직접 걸리는 건 0건). 상품번호↔옵션이 1:N이어도 sellc 원가는 갈리지 않는다(실측 208/208 단일). 갈리면 비싼 쪽 채택 + 경고(싼 쪽은 순이익 과대).
  - **환율 주석**: Jino가 말한 3,400과 sellc의 3,300 차이는 **환율 변동에 따른 원가 개정**이다 — sellc 값이 현행이다.

  **라이브 증거(2026-08-03 23:0x, prod 배포 후)**
  - 계정 원가 출처 분해: **sellc 14,708,222 / auto_map 2,165,476** (sellc가 87.2%).
  - 계정 `net_profit` 37,635,542 → **34,555,757**(−3,079,785). 검산: 전환 전 원가 13,793,913 + 3,079,785 = 16,873,698 = 전환 후 원가 합(14,708,222+2,165,476)과 **원 단위 일치**.
  - SKU 순이익 도달 88.28%(sellc 80.60% + 자동추정 7.34%).
  - 계정 원가 커버리지 94.71% → **76.14%**. ★하락은 결함이 아니다 — 틀린 매핑(76350897·76350898, 발주 14,448,000원분)을 **제거했고 sellc에도 등록이 없어** 원가 미상으로 남았기 때문이다. 잘못된 원가를 지우면 커버리지는 떨어지고 그게 정직한 상태다.
  - ★사업적 발견: `62922000`(폴드7 무광택)이 sellc 원가로는 **순이익 −252,725원(적자)**이다. 자동매핑 원가(3,890)로는 +116,215원 흑자로 보였다 — 원가 출처 하나가 흑자를 적자로 뒤집는다.

  **남은 것**
  - 프론트(출처 배지·커버리지 분해)는 **미배포** — 병행 세션이 `NaverAdModifications.tsx`를 수정 중이라 `npm run build`가 그 파일에서 실패한다(내 코드 문제 아님). 그 세션의 빌드가 통과하면 올린다.
  - sellc에 없는 상품(76350897·76350898 등)은 sellc에 등록하면 바로 해소된다 — 코드 작업이 아니다.
