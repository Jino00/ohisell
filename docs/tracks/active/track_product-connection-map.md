...
- [x] S6b-온라인매핑 UI 보강 (2026-07-04) — 미매핑 옵션ID 목록 펼침+원클릭 연결, prod 배포·라이브검증 완료
- [ ] S6b-데이터 오픽스(WING1/RG1) 실제 매핑 입력 — Jino가 온라인 UI 또는 갱신 엑셀로 직접 수행(코드 작업 아님)

## S6b-온라인매핑 UI 완료 기록 (2026-07-04)
- **배경**: S6b(오픽스 매핑 결손 보강)를 "Jino가 갱신 엑셀을 만들어와야 함"으로만 안내했으나, 실은 S4에서 이미 만든 온라인 매핑 CRUD(`POST/PATCH/DELETE .../mappings`)로 화면에서 직접 채울 수 있음을 재확인(엑셀은 필수가 아니라 대량 갱신 시의 한 방법일 뿐). 유일한 진짜 공백은 "어떤 옵션ID가 미매핑인지 목록"이 화면에 안 보이던 것(개수만 표시).
- **구현**(프론트 전용, 백엔드 무변경 — 기존 `GET /api/products/mapping-coverage`+`POST .../mappings` 재사용): 채널 헤더의 "미매핑 N" 카운트를 클릭하면 그 채널의 미매핑 옵션ID 목록(옵션ID+주문건수)이 펼쳐짐 → 항목의 "연결할 상품 찾기" 클릭 → 안내 배너 표시 + 검색창으로 상품 좁히기 → 대상 채널 열에 나타나는 "↳ 이 옵션 연결" 버튼 클릭 → 옵션ID가 미리 채워진 매핑 추가 폼이 열림(판매가만 입력 후 저장).
- **검증**: `npx tsc -b --noEmit` clean. dev DB 사본으로 실제 브라우저 라이브 e2e(원칙22) — 미매핑 목록 펼침→연결할 상품 찾기→OHI-0001 행에 실제 옵션ID(`94156365627`) 매핑 신규 생성 확인, 매핑 카운트 20→21·미매핑 2→1 정확 반영, `mapping_source=manual` provenance 정상.
- 커밋 `6265df9`(main 직접 push) → 프론트 빌드+rsync로 prod 배포·200 확인.
- **참고**: S6b-데이터(실제 오픽스 WING1/RG1 매핑 입력)는 이제 Jino가 `/product-connection-map` 화면에서 직접 채울 수 있음(별도 코드 작업 불필요). 엑셀 업로드도 여전히 가능(대량 갱신 시 편의).
...
## 📍 현재 진행 단계
S1~S5 전부 main 머지(PR #1~#4) + **S6a prod 배포·라이브 self-verify 완료** + **S6b-온라인매핑 UI 완료**(2026-07-04, 전부 main 직접 push + prod 배포). prod가 이 트랙 전체(6/22 이후 미배포 상태)를 스코프드로 따라잡음 + 라이브에서 실운영 버그(순환소수→conservation_ok 오판→SKU행 은폐) 발견·즉시 수정(`c3dae2a`+`0f879ec`) + 미매핑 옵션 온라인 연결 UI 추가(`6265df9`). **이 트랙의 코드 작업은 전부 완료**. 남은 건 Jino의 실제 데이터 입력(오픽스 매핑) 하나뿐.

## ▶️ 다음 액션
1. ~~S3 백엔드~~·~~S4 탭1 UI~~·~~S5 탭2 UI~~·~~S6a prod 배포·라이브 self-verify~~·~~S6b 온라인매핑 UI~~ **전부 완료**.
2. **S6b-데이터(유일한 남은 항목, 코드 아님)**: 오픽스(WING1/RG1) 매핑 결손 보강 — Jino가 `/product-connection-map` 화면에서 직접 매핑을 추가하거나(신규 온라인 UI), 갱신된 마스터 엑셀을 업로드하면 끝. 둘 다 가능, 코드 변경 불필요.
