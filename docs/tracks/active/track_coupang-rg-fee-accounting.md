...
## 다음 액션
- **S6-auto prod self-verify**: Wing 쿠키가 DB에 있으면 `POST /api/coupang/ops/rg/settlement/auto-download` 실행해 라이브 동작 확인(원칙22). 쿠키 없으면 수동 업로드(`POST /api/coupang/ops/rg/settlement/upload-xlsx`)로 계속 운용.
- **S6-auto scheduler 등록**: S4에서 만든 `sync_coupang_rg_settlement_job` 옆에 `auto_download_rg_settlement_job`(주간 또는 정산 주기별) 등록.
