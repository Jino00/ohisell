...
- **S8 후속(선택)**: ★2026-06-15 라이브 재감사 — size_mismatch_high는 이제 **1건**(이전 4건은 PRODUCT_SIZE_COMPARISON 실측 사이즈 자동수집되며 해소). 남은 1건 = 아이패드미니필름(91313543029, 등록 극소형 세변합 60.5cm vs 배송청구 주문당 4,050원=대형1 정합 3배, size_source=registered_dims 실측 미확보). **Jino 결정(2026-06-15): 자동해제 대기** — 다음 입고 시 실측 사이즈 수집되면 자동 판가름(ⓐ극소형 확정→과오청구 / ⓑ대형1 확정→정당), 코드변경 없음. 프론트 UI(로켓그로스 탭에 감사 뷰) 추가는 미정.
  ...
  - **같은 원인의 오탐 4건 전부**: `measured_vs_billed_mismatch` WING2 1건(91313543029) + WING1 3건(94156664759·72·81, 오픽스 필름 — 주기 2개×1,900=3,800을 주문 1건으로 나눔). 주문 수집 창(2026-05-11~)으로 조회하면 **4건 모두 플래그 0**(라이브 확인). → **S8 감사 결함**: per-unit 정규화가 분자·분모의 커버리지 일치를 확인하지 않는다. 수정 방향(Jino 선택 대기)은 ⓐ주문 데이터 없는 정산주기를 분자에서 제외 ⓑ커버리지 불일치 시 `unit_unknown`으로 강등 ⓒ주별(주기별) 단가로 판정. 읽기 전용·net_profit 무영향(D-17).
