...
- [x] **E1a expert_desk (ohisell 자족) + 콘솔 + codex review — 완료(2026-07-09, T1~T9 전체)** — 상세 §8. Ava 페르소나로 08:05 배치 검토(Opus), 평결 저장·콘솔, 검증가능예측 D+7/14 로컬 성적표. AI_office·실 claude 무의존(주입경계 TDD).
  ...
  - [x] **T9 prod 사본 e2e(가짜 reviewer) — E1a 최종 태스크 완료** (2026-07-09, sonnet). prod DB scp 읽기전용 사본 → 미배포 마이그레이션 4개(`w7x8y9z0a1b2/x8y9z0a1b2c3/y9z0a1b2c3d4/z0a1b2c3d4e5`) alembic.ini 임시포인팅 적용 후 즉시 원복(원칙20 절차). **prod에 NaverProposal 0건**(D-3 발견: proposal_pipeline이 prod에 배포된 적 없음, F0b 필요 재확인) — F2 89K 재검증과 동일 원칙으로 `proposal_pipeline.run_daily`를 사본에서 먼저 실행해 **실제 prod 광고 데이터로 실제안 2건 생성**(anomaly_feed 소진급증 1건 + account_brief 1건, Slack webhook 미설정이라 안전하게 no-op). 그 위에 `expert_desk.run_daily(db, invoke=가짜)` 실행: **4단계 전부 ok**(grade_due_predictions·briefing_builder·ava_reviewer·expert_ledger), 평결 3행(agree 1·insufficient_evidence 1·commentary 1) 생성, **스키마위반 0**. 라우터 3개(`/proposals`·`/expert-reviews`·`/expert-scorecard`) 전부 실제 데이터로 curl 검증 — expert_verdict 조인·평결 목록·정직 라벨("아직 채점된 예측이 없습니다") 전부 정확. grade_due_predictions 채점 단위테스트는 T4에서 이미 15개로 커버(4-outcome+멱등+C3경계+exact recompute) — T9은 신규 코드 없이 순수 검증이라 추가 없음. **codex review**: T1~T8 매 태스크에서 이미 누적 실행·전부 pass(P1 다수 발견·수정 포함) — T9은 신규 코드 없어 별도 codex 세션 불필요. 검증 후 prod 사본 즉시 삭제(prod 원본 무접촉). pytest 901 유지. **E1a 전체(T1~T9) 완료.**
