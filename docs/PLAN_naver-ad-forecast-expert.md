...
- [x] **E1a expert_desk (ohisell 자족) + 콘솔 + codex review — 완료(2026-07-09, T1~T9 전체)** — 상세 §8. Ava 페르소나로 08:05 배치 검토(Opus), 평결 저장·콘솔, 검증가능예측 D+7/14 로컬 성적표. AI_office·실 claude 무의존(주입경계 TDD).
  - [x] **T1 마이그레이션+모델** — 완료(2026-07-09, sonnet, TDD). 신규 마이그레이션 `z0a1b2c3d4e5`(2테이블: `naver_expert_review_run` 원장 + `naver_expert_review` verdict child, run_id/proposal_id FK) + 모델 `NaverExpertReviewRun`/`NaverExpertReview` + 모듈 상수 4종(verdict/outcome/source/run_status, C2). 모델 테스트 7개(run↔verdict FK 링크·재실행=새run 이력보존·commentary NULL proposal_id·reject에도 checkable_prediction 허용·insufficient_evidence 정직평결·컬럼폭 회귀가드). **codex review**: [P1] `verdict String(20)`이 `insufficient_evidence`(21자)를 못 담는 실버그 발견 → `String(24)`로 즉시수정+회귀테스트 추가. P2 2건(status/source 기본값 스타일, created_at nullable 생략)은 기존 `naver_forecast_*` 마이그레이션과 동일 컨벤션이라 근거 명시 기각(원칙19). 실sqlite로 upgrade/downgrade/재upgrade 라운드트립 별도 검증(원칙22, pytest는 create_all이라 alembic 자체는 안 거침). pytest 808→815. 커밋 `c807971`.
  - [x] **T2 `expert_briefing_builder`** — 완료(2026-07-09, sonnet, TDD). 신규 `expert_briefing_builder.py`(SA1, 결정적·LLM무호출): 오늘 pending 제안 전체(NaverEntity 조인 라벨) + 진단보드 요약(`diagnosis.build_diagnosis` 15일창 재사용) + forecast 예측vs실측 롤업(채점된 것만, recent-N) + 최근 trigger 이벤트(`NaverProposal` trigger_* 타입, recent-N) + 로컬 성적표(`NaverLearningState` 읽기전용). **토큰가드 신규 도입**(이 코드베이스 최초 전례): pending_proposals 직렬화 길이 초과 시 오래된(id 작은) 것부터 절삭+`log.warning`+`truncated` 필드 노출(no silent cap). 테스트 11개(결정성·pending필터·entity명 해석·토큰가드 절삭+로깅·forecast/trigger recent-N 바운드·채점됨만·성적표 읽기전용·진단 unavailable 정직노출). **codex review**: private 상수(`_PROPOSAL_TYPE_PACING`/`_CPC`) cross-module import 지적 → `trigger_watch.py`에서 public 승격(기존 `BACKFILL_SENTINEL_ADGROUP` 전례와 일치, 동의·수정). "오늘 pending 필터 없음(created_at 미제한)" 지적은 근거 명시 기각 — 14일 자동만료(`proposal_pipeline._expire_stale_pending`)로 백로그가 이미 유한하고, 토큰가드는 실제로 오래된 것부터 버려 최신을 보존(codex의 "오늘 것 밀어냄" 주장은 코드와 반대 — codex 오독). board 요약 lossy(list→count만) 지적은 7개 보드 내부구조를 깊이 모른 채 미리보기를 지어내면 추정이 되므로 T3(리뷰어 프롬프트 실제 요구사항 확인) 이후로 보류. `db.no_autoflush` 제안은 codex 스스로도 "prod SessionLocal이 이미 autoflush=False라 현재 버그 없음" 확인 → 존재하지 않는 시나리오 방어코드는 추가하지 않음(기각). pytest 815→826. 커밋 `0675b7b`.
  - [x] **T3 `expert_llm`+`ava_reviewer`** — 완료(2026-07-09, sonnet, TDD). `expert_llm.py`: AI_office `claude_cli.py::call_claude_cli` 린 포팅(cost_guard/chokepoint/usage_db 미포함, C1 출처주석) — stdin 프롬프트(ARG_MAX 회피)+ANTHROPIC_API_KEY 제거(OAuth)+재시도(t→×1.5→×2)+`--output-format json` 파싱+펜스코드블록 regex JSON 추출. `ava_reviewer.py`(SA2): 브리핑→Ava 평결, `invoke` 주입경계(기본=`_invoke_claude`, 테스트=가짜). **강한 스키마 검증**(누락/중복/여분 proposal_id·무효 verdict·commentary↔proposal 혼동 전부 거부) — 위반 시 1회 재시도 후 `status="degraded"`(빈 평결, 조작·환각 보정 금지). 페르소나=E1a 로컬 Ava 상수 stub(AI_office factory.py 역할 문구 그대로 반영). **codex review**: [P1] `returncode=0`인데 stdout이 JSON봉투가 아니면 `JSONDecodeError`가 그대로 새어나가던 것 발견 → `RuntimeError`로 통일(즉시수정+회귀테스트). P2 2건(`reasoning` 필수필드 미검증, bool/float `proposal_id`가 멤버십비교 통과 가능) 동의·수정. 프롬프트 인젝션 잔여위험 지적은 codex도 P1 아니라 판단 → 보류(T4 ledger가 verdict를 그대로 신뢰하지 않고 D+7/14 실측 채점으로 검증하는 구조가 완충). `invoke()` 예외 비흡수는 `learning_loops.py` 패턴(Harness가 catch, SA는 안 함)과 일치해 유지. 테스트 25개 신규. pytest 826→851. 커밋 `21abec7`.
  - [x] **T4 `expert_ledger`(record + grade_due_predictions)** — 완료(2026-07-09, sonnet, TDD). `record(db, as_of, review)`: ava_reviewer 결과를 run 원장+평결 child로 저장, 멱등 키=(as_of, briefing_hash)(진짜 재실행=다른 브리핑=새 run 이력보존 / 동일 호출 재시도=no-op). checkable_prediction 있는 평결만 verify_date=as_of+7+outcome=pending. 총평=proposal_id NULL 행 1개(A1, run당 자연 dedup). `grade_due_predictions(db, today)`: verify_date 도래한 pending 평결을 before/after 7일창 NaverAdDaily 실측 대조로 4-outcome 채점 + `NaverLearningState(scope=expert)` 성적표 갱신(채점가능 신호 0건이면 성적표 안 건드림, 정직 경계). ava_reviewer.py에 `model`/`prompt_version`/`briefing_hash` 반환 필드 보강(record가 provenance 채우는 데 필요). **⚠️C3 자문경계**: NaverProposal import 자체 안 함(구조적) + 런타임 가드(같은 세션에 커밋 안 된 NaverProposal 변경 있으면 RuntimeError) 이중 방어. **codex review — 진짜 P1 5건**: ①세션 공유 시 커밋 전 NaverProposal 변경까지 같이 흘러나갈 수 있던 C3 허점 → 런타임 가드로 즉시수정 ②무변동(before==after)을 "down"으로 단정해 down예측이 거짓 correct가 되던 버그(주석·코드 불일치) → unverifiable로 수정 ③미지 pred_target_type이 campaign 분기로 새던 것 → 화이트리스트 검증 ④채점커밋·성적표커밋 비원자적(중간실패 시 데이터 유실) → 단일 트랜잭션 병합(수정 중 파생된 autoflush 버그도 테스트가 즉시 발견해 flush() 추가로 해결) ⑤반올림 비율로 이전 correct 수를 역산하던 표류 위험 → 매번 outcome 테이블 정확 재집계(ground truth)로 교체. P2 1건(verify_date 타입힌트)은 코드베이스 전체 컨벤션(NaverProposal.verify_date 등)과 일치해 기각. 테스트 21개 신규. pytest 851→876. 커밋 `4e60a6f`.
  - [x] **T5 `expert_desk` 하네스** — 완료(2026-07-09, sonnet, TDD). `run_daily(db, *, today=None, invoke=None)`: learning_loops.py식 단계격리(stage1 grade_due_predictions → stage2 briefing_builder.build → stage2.5 빈제안 가드 A2(pending 0건이면 claude 미호출·stage3/4 skip) → stage3 ava_reviewer.review(invoke 주입경계, 미전달 시 실 claude 폴백) → stage4 expert_ledger.record), 각 단계 독립 try/except. **codex review**: P1 0건("learning_loops 패턴을 정확히 따랐다" — 단계격리·A2 배치·rollback 비대칭 3개 질문 전부 codex가 직접 추적해 문제없음 확인). P2 2건: `result["grade"]`가 stage1 실패 시 키 자체가 없어 호출측 KeyError 위험 → `None` 초기값으로 수정. T4의 C3 런타임가드가 stage1/4에서 걸리는 게 의도된 동작(방어막이 작동 중이라는 신호)이라는 점을 docstring에 명시. 테스트 8개 신규(전체성공·A2 검증·단계별 독립실패·grade 실패시에도 result 키 존재·오늘 기본값). pytest 876→884. 커밋 `2e8b8ae`.
  - [x] **T6 라우터** — 완료(2026-07-09, sonnet, TDD). `GET /api/naver/ad/expert-reviews?as_of=&proposal_id=&limit=` 신규(단순 read). `GET /proposals` 응답에 `expert_verdict` 요약 조인 — 제안별 가장 최근 완료(status=ok) run의 평결(배지용), degraded/skipped/failed run 제외. **codex review**: [P1] "as_of 최근 완료 run" 스펙인데 정렬이 run.id에만 의존해 백필/재처리 시나리오에서 진짜 최신이 아닌 걸 고를 수 있던 버그 발견 → `NaverExpertReviewRun.as_of`를 id보다 우선 정렬 기준으로 추가 + 회귀테스트. P2 1건(limit 파라미터 노출) 반영, P2 1건(proposal_id 인덱스 부재)은 마이그레이션 필요+라우터 전용 태스크 스코프 밖이라 보류(codex도 비긴급 판단). 기존 `/proposals` HTTP 라운드트립 테스트 회귀 없음(원칙22). 테스트 11개 신규. pytest 884→896. 커밋 `90825f7`.
  - [x] **T7 크론 08:05 등록** — 완료(2026-07-09, sonnet). `scheduler_service.generate_expert_desk_job`(`5 8 * * *`) — 기존 잡 3개(generate_naver_proposals·run_naver_learning_loops·run_naver_forecast_engine)와 동일 패턴, 별도 단위테스트 없음(코드베이스 기존 컨벤션 — 위임 대상 expert_desk.run_daily는 이미 전부 테스트됨). **codex review**: [P1] "08:00 제안생성이 5분 안에 못 끝나면 그날 신규 제안 일부를 놓칠 수 있다" 지적 → 검토 결과 실패모드=유실이 아니라 지연(briefing_builder가 매일 pending 전체를 읽어 다음날 08:05에 자동 재포함, 데이터 손실 없음). 08:05는 계획서에 이미 Jino 승인된 값이라 이 태스크에서 임의 변경 안 함 — docstring에 트레이드오프 명시(실측 후 필요시 조정, 추정 금지). P2 1건(defaults 리스트 가독성 순서) 반영. pytest 896 유지(테스트 수 변화 없음, 기존 컨벤션). 커밋 `c107617`.
  - [x] **T8 프론트 + 성적표 엔드포인트** — 완료(2026-07-09, sonnet, TDD+라이브 검증). 조사 중 T6가 놓친 부분 발견: `GET /expert-scorecard` 엔드포인트 부재(expert_ledger가 이미 쓰는 NaverLearningState scope=expert를 읽을 라우트가 없었음) → 신규 추가(표본 20건 미만 시 정확도% 대신 정직 라벨). `api.ts`에 `NaverAdProposal.expert_verdict` 필드 추가(T6부터 백엔드가 이미 보내던 필드인데 TS 타입만 누락됐던 드리프트 발견·수정) + `fetchNaverExpertReviews`/`fetchNaverExpertScorecard` 신규. `NaverAdOptimizationConsole.tsx`에 제안 카드 평결 배지(✓/⚠/✗/?) + "Ava의 검토" 섹션(총평+성적표). **codex review**: P1 0건. P2 2건 반영 — 성적표 쿼리 scope_key="all" 필터 누락(미래 세분화 시 오채점 위험) 추가, 표본 부족 시에도 정확도%가 라벨보다 먼저 노출되던 것을 라벨 있을 땐 정확도% 자체를 숨기도록 강화. **라이브 검증(원칙22)**: 스크래치 SQLite+스케줄러 미포함 미니 FastAPI 앱(외부 API 크레덴셜 위험 회피)+실제 vite dev 서버로 브라우저 렌더링 확인 — 배지 3종·총평·정직라벨 전부 정상, 콘솔에러 0, 신규 엔드포인트 2개 200. `npx tsc -b --noEmit` 통과. 테스트 5개 신규. pytest 896→901. 커밋 `b76091d`.
  - [x] **T9 prod 사본 e2e(가짜 reviewer) — E1a 최종 태스크 완료** (2026-07-08, sonnet, 라이브 검증). **F0b(prod 실배포)와 통합 진행**(Jino "모두 진행해줘" 승인 하에 브랜치를 main에 fast-forward merge 후 실배포). prod DB 백업(`/home/ubuntu/ohisell_bak/naver-ad-full_20260708_111513/`) → 신규/변경 파일 58개(F0/F1/F2/듀얼모드+E1a 전체, prod가 4개 마이그레이션 뒤처져 있었음이 이번에 발견됨) tar 패키징+전송+원격추출 → **sha256 전수 검증**(1차 시도에서 27개 파일이 bash `while read`+`ssh` stdin 충돌로 조용히 누락된 걸 발견·재전송, macOS tar AppleDouble 사이드카(`._*.py`)가 alembic 파싱을 깨던 것도 발견·정리 — 둘 다 failures.jsonl에 기록) → 마이그레이션 4개(`w7x8y9z0a1b2/x8y9z0a1b2c3/y9z0a1b2c3d4/z0a1b2c3d4e5`) 실prod DB에 적용 → pm2 재시작(스케줄러 4개 신규 크론 정상 등록 확인, next_run_time 실측) → 프론트 빌드+rsync 배포. **라이브 self-verify(원칙22, 전부 외부 HTTPS로 확인)**: `/proposals`·`/expert-reviews`·`/expert-scorecard` 3개 신규 엔드포인트 실 데이터로 200 확인. **prod 실제 pending 제안 1건 발견**(`account_brief`, target_type=account — D-3 관찰모드로 이미 존재하던 실데이터, 새로 생성한 것 아님). 이 실제안으로 prod 사본(scp 재다운로드, 마이그레이션 반영된 상태)에서 `expert_desk.run_daily(db, invoke=가짜)` 실행: **4단계 전부 ok**(grade_due_predictions·briefing_builder·ava_reviewer·expert_ledger), 평결 2행(agree 1 + commentary 1) 생성, **스키마위반 0**, C3 경계 재확인(proposal.status 불변). 라우터 재검증: expert_verdict 조인·평결목록·정직라벨("아직 채점된 예측이 없습니다") 전부 정확. grade_due_predictions 채점 단위테스트는 T4에서 이미 15개로 커버(4-outcome+멱등+C3경계+exact recompute) — T9은 신규 코드 없이 순수 검증이라 추가 테스트 없음. **codex review**: T1~T8 매 태스크에서 이미 누적 실행·전부 pass — T9/F0b는 신규 코드 diff 없어(배포+검증 작업) 별도 codex 세션 불필요. prod 사본 검증 직후 즉시 삭제. pytest 901 유지(회귀 0). **E1a 전체(T1~T9) + F0b(배포) 완료.**
- [x] **F0b 잔여: prod 캠페인 180일 백필 — 완료(D-NAO-33, 2026-07-08, 이어지는 세션, Sonnet)**. prod 서버 SSH 접속 → 실DB 사전 백업(`ohisell_bak/naver-ad-campaign-backfill_20260708_115522/`) → `campaign_backfill.backfill_campaign_daily`를 prod 실DB에 직접 실행(스크래치 아님, 실 네이버 `/stats` API 호출). 결과: 43캠페인·7,740행·2026-01-09~07-07·cost합 80,335,231원(F0a 스크래치 실행과 완전 일치). 완료기준 둘 다 충족: ①07-04~07-07 실단위 집계 백필 전/후 byte-identical ②겹치는 10일 재백필 멱등 확인(sentinel 행 7,740 유지). 상세: 트랙 D-NAO-33. **F0b 완전 종결.**
- [ ] **E1b Ava 연동** — ava_client(wisdom pull 브리핑 주입 + observe push) + 실 claude 어댑터 스모크. **AI_office는 다른 레포/프로젝트** — 이 세션(ohisell)에서 불가, AI_office 쪽 별도 세션 필요.
- [ ] E2 부분 게이트 (보류 — 반자동 전환 결정과 동기, Jino 결정 대기)
