...
- [ ] **F0b 잔여: prod 캠페인 180일 백필** — F0b의 배포·마이그레이션·라이브검증은 위 T9에서 완료. **캠페인 grain sentinel 백필(F0a와 동일 작업을 prod 실DB에)만 남음** — 이게 없으면 forecast_engine의 캠페인 grain 모델이 계속 `fallback` 상태(F2 89K 재검증에서 이미 확인된 상태). 별도 세션에서 진행.
