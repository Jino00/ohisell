...
- [ ] **E1a expert_desk (ohisell 자족) + 콘솔 + codex review** — 상세 §8. Ava 페르소나로 08:05 배치 검토(Opus), 평결 저장·콘솔, 검증가능예측 D+7/14 로컬 성적표. AI_office·실 claude 무의존(주입경계 TDD).
  ...
  - [x] **T7 크론 08:05 등록** — 완료(2026-07-09, sonnet). `scheduler_service.generate_expert_desk_job`(`5 8 * * *`) — 기존 잡 3개(generate_naver_proposals·run_naver_learning_loops·run_naver_forecast_engine)와 동일 패턴, 별도 단위테스트 없음(코드베이스 기존 컨벤션 — 위임 대상 expert_desk.run_daily는 이미 전부 테스트됨). **codex review**: [P1] "08:00 제안생성이 5분 안에 못 끝나면 그날 신규 제안 일부를 놓칠 수 있다" 지적 → 검토 결과 실패모드=유실이 아니라 지연(briefing_builder가 매일 pending 전체를 읽어 다음날 08:05에 자동 재포함, 데이터 손실 없음). 08:05는 계획서에 이미 Jino 승인된 값이라 이 태스크에서 임의 변경 안 함 — docstring에 트레이드오프 명시(실측 후 필요시 조정, 추정 금지). P2 1건(defaults 리스트 가독성 순서) 반영. pytest 896 유지(테스트 수 변화 없음, 기존 컨벤션). 커밋 `c107617`. **다음 = T8 프론트**(제안 카드 평결 배지 + "Ava의 검토" 패널).
