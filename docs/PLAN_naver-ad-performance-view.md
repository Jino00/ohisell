...
- [x] ✅ P2-6 pytest 37건 신규(날짜 소스 분기·비교 산출·뱃지 4상태·필터·암전·수집공백·ID 누출 0,
      전체 3,881 통과) + vitest 11건 신규(전체 88 통과)
- [ ] ⏳ P2-6b `/codex review` (원칙 19)
...
- **03 캠페인 상세**: 기준선 BEP 1.6676 / 목표 1.9178, 30일 시계열에서 **BEP 아래로 내려간 날 6일**
  ...
  데이터가 그렇게 말한다.
- **07-27 예산 곡선**: 46곡선. **이날은 암전 구간이 0건**이다 — 일예산을 넘긴 캠페인
  (01. 갤럭시_지문방지_TPU 367,818/300,000)도 23시까지 계속 집행됐다(네이버 초과 집행).
  검출기 자체는 21일 전수 스캔에서 **07-14 맥세이프카드케이스 22~23시 암전**을 잡았다
  (99,492/100,000 = 99.5% 도달 후 2시간 증분 0) → "오후 10시쯤 하루 예산을 다 써서 그 뒤로
  2시간 동안 광고가 나가지 않았습니다."
