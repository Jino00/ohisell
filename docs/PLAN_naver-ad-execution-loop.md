...
- [x] X1b T2 guardrail_gate SA — **완료(2026-07-10, Sonnet 구현, TDD)**: 순수 판정 함수 `check(proposal, context, *, now)` — bid_up/bid_down/growth_bid_up/pause/resume 5개 유형. 공통(전 유형): 쿨다운(`_COOLDOWN_HOURS=5` — D-NAO-19 미문서화 수치, trigger_watch.TRIGGER_COOLDOWN_HOURS 재사용해 정직 라벨) + 일일 변경 건수 상한(`_MAX_DAILY_CHANGES=3`, 신규 상수·정직 라벨). bid 전용: 클램프(70~100,000·10원단위, writer와 이중 방벽)→변경폭 ±15%(growth_bid_up은 D-NAO-20-③ 면제)→[up 방향만] 스톱로스(`growth_sweeper.STOP_LOSS_CLICK_MULTIPLE` 재사용, target_bid×10)→BEP미달 증액금지→일예산 상한 불가침(cost_today≥daily_budget). bid_down은 손실축소 방향이라 스톱로스·BEP·일예산 검사 면제(의도적 비대칭 — 그게 목적). lock 전용: target_lock 구조 검증만. `current_bid`/`target_bid`/`target_lock` 부재 시 전부 fail-closed(검증불가=차단). 테스트 29개 신규(1077→1106 passed, 회귀 0). **codex review**: 2건 발견·즉시수정(P2 — ①`bid_down`에 인상 방향 target_bid가 와도 절대변경폭만 봐서 통과·up전용 스톱로스/BEP/일예산 검사까지 우회 가능 ②`pause`에 target_lock=False, `resume`에 target_lock=True가 와도 bool 타입 검증만 통과 — 둘 다 방향 불일치 fail-closed 차단 추가, TDD 6개 신규). 테스트 총 35개(1106→1112 passed). 3라운드에서 P2 1건 추가 발견·즉시수정: `daily_budget=0`을 budget_allocator 기존 관행(daily_budget>0 필수, D-3)과 다르게 그대로 비교해 uncapped 캠페인(dailyBudget=0=미설정)의 정상 bid_up까지 차단하던 버그 — `daily_budget > 0` 조건 추가. 테스트 1개 신규(1112→1114). **4라운드 codex review 최종 PASS**(0 findings — "did not identify any discrete regressions in the changed migration, model fields, writer helpers, or guardrail gate logic"). T1·T2 완결.
- [x] X1b T3 proposal_writer target_bid 저장 + 정지·재개 생성기 — **완료(2026-07-10, Sonnet 구현, TDD)**: ①`_bid_proposal`/`_growth_proposal`이 `recommended_bid`를 `target_bid` 컬럼에 구조화 저장(negative_keyword 격상은 입찰목표 없어 None). ②신규 `account_diagnosis.pause_candidates`(WEB_SITE 등록 키워드 status='on' 중 창 내 전환 0건+누적비용≥스톱로스절대액[현재입찰×LOW_CLICK_THRESHOLD, D-NAO-20]) + `resume_candidates`(우리가 정지시킨[change_log action='pause', proposal_id 有 — Jino 수동정지는 proposal_id 없어 제외] 키워드 중 정지 직전 창 보정ROAS≥현재목표, D-NAO-16 "BEP 개선". 정직 경계: 계절성 회복·CPC 하락은 미구현·§8 승계 큐 후보) → `diagnosis.py`에 신규 보드 2개 배선. ③`proposal_writer`에 `_pause_proposal`(target_lock=True)·`_resume_proposal`(target_lock=False) 빌더 + `build()`에 ours 필터 적용 루프 배선(실행형이라 진단보드와 달리 optimizer 무관 아님, D-NAO-13). 테스트 22개 신규(1114→1136 passed, 회귀 0). **codex review**: P2 2건 발견·즉시수정 — ①`pause_candidates`가 키워드 자신만 status='on' 확인해, 부모(광고그룹·캠페인)가 off인데 키워드가 개별 on인 경우(entity_sync 비캐스케이드, F2a D-NAO-27과 동일 근거)도 정지 제안을 만들어 부모 재개 후에도 키워드만 영구 잠길 위험 → `_on_adgroup_ids()` 부모체인 교차확인 추가 ②`resume_candidates`가 action='pause'만 보고 max(changed_at)를 구해, 정지→우리재개→(수동or다른사유)재정지 이력에서 옛 우리 정지를 잘못 채택할 위험 → pause·resume 통합 조회로 진짜 최신 잠금변경을 가려 그게 우리 pause일 때만 진행. 테스트 9개 신규(1136→1141 passed). **2라운드**: P2 1건(N+1 성능) 발견·즉시수정 — `resume_candidates` 루프가 후보마다 `_keyword_rows()`(WEB_SITE 전 키워드 GROUP BY 집계)를 호출해 O(재개후보수×전체키워드행)로 스케일. keyword_id를 SQL WHERE에 직접 걸어 그 키워드 행만 집계하는 타겟 쿼리로 교체 + adgroup_id는 ad_daily 재조회 대신 이미 로드한 NaverEntity.parent_id 사용(부수 개선). 회귀 0(1141 유지, 기존 32개 테스트로 동작 보존 확인). **3라운드**: P2 1건(캠페인 override 무시) 발견·즉시수정 — `resume_candidates`가 계정 단일 `target_roas`만 받아 `/campaign-settings`의 `target_roas_override`가 실제 판정에 반영 안 됨(compute_bid_sims의 `_make_target_roas_resolver`가 2026-07-07 라이브검증에서 이미 한 번 고친 것과 동일 재발 버그 패턴). `target_roas: Decimal` 파라미터를 `target_roas_resolver`(campaign_id→Decimal 콜러블)로 교체, `diagnosis.py`(Harness)에 `_target_roas_resolver()`(회차 내 캐싱, `campaign_target_resolver.resolve_target_roas` 재사용)를 신설해 주입. 테스트 2개 신규(1141→1143 passed, 회귀 0). **4라운드**: P2 1건 — pause/resume이 `naver_execution_harness._ACTION_BY_PROPOSAL_TYPE`에 아직 매핑되지 않아 `/proposals/{id}/execute`가 "정보성 제안"으로 오분류·거부, 만료까지 적체됨. **의도적으로 T4로 이관(수정 아님, T4 자체가 이 매핑을 만드는 태스크)** — 별도 임시 배선 후 T4에서 다시 정리하면 중복 작업. T4에서 이 갭이 해소됨을 완료기준으로 삼는다.
- [x] X1b T4 execution_harness 개방(update_bid·set_user_lock 실행자) + MOP 충돌감지(D-NAO-13) — **완료(2026-07-10, Sonnet 구현, TDD)**: ①`OPEN_ACTIONS`에 update_bid·set_user_lock 추가(D-NAO-16 2·3단계 개방, 예산만 스코프 밖) ②`_ACTION_BY_PROPOSAL_TYPE`에 pause/resume→"set_user_lock" 매핑(단일 실행자 공유, target_lock 값으로 방향 구분) ③신규 `_build_guardrail_context()`(라이브 재조회+30일 창 실적집계[account_diagnosis.keyword_window_agg 재사용]+diagnosis.correction_factor[비공개→공개 승격]+캠페인별 target_roas[campaign_target_resolver]+NaverHourlySnapshot 당일 소진+change_log 쿨다운/일일카운트, 전부 fail-closed 폴백) ④`_execute_update_bid`/`_execute_set_user_lock`(claim→구조가드→guardrail_gate.check()→쓰기→검증→change_log, `_execute_add_negative_keyword`와 동일 골격 — `_guard_failure`/`_claim_executing` 공유 헬퍼로 추출) ⑤`_detect_external_change()`(D-NAO-13 MOP 충돌감지 — 우리 마지막 change_log.after_value와 방금 재조회한 라이브 before가 다르면 rationale에 경고 부착, 차단 아님) ⑥`real_write_blocker()`에 구조 판정 ④⑤ 추가(가드레일 라이브 재조회는 실행 시도 시점에만 — 목록 조회마다 API 호출하면 콘솔 로딩이 승인 건수만큼 네이버 API를 유발하므로 의도적으로 제외, 설계 결정). **adgroup 단위 입찰·정지는 미구현**(fail-closed 가드, ref 27 §4 문서화됐으나 미구현 — shopping_group_bep 제안은 이 가드에서 걸림, 정직 경계·§8 승계 큐 후보). 테스트 34개 신규(1143→1177 passed) + 기존 5개 회귀(delegable_types·라우터 executable 판정이 bid_up을 "미개방 예시"로 쓰던 것을 budget_up으로 교체 — 실패 아니라 X1b가 의도한 스코프 확장의 자연스러운 결과, delegable_types() 자체 docstring이 이미 예고). 회귀 0. **codex review**: P2 2건 발견·즉시수정 — ①**치명적**: `_execute_set_user_lock`이 pause/resume 둘 다 change_log에 `action="set_user_lock"`으로 기록(update_bid가 bid_up/bid_down/growth_bid_up을 묶는 것과 동일 관례)하는데, `resume_candidates`는 여전히 `action.in_(("pause","resume"))`로 조회 — 문자열이 영영 일치하지 않아 정지→재개 루프 전체가 작동 불능이었음(Jino "완벽하게 작동" 지시의 핵심 실패). 방향 판별을 action 문자열이 아니라 `after_value`의 실제 userLock 값으로 바꿔 해결(after_value 파싱불가/누락은 fail-closed 제외) ②`_build_guardrail_context`의 쿨다운·일일카운트 쿼리가 outcome 무관 전 change_log 행을 셈 — dry-run(outcome=None)·사전가드실패(outcome='failed')는 네이버 상태를 안 바꿨는데도 다음 실행을 차단할 수 있었음 → `outcome=='executed'` 필터 추가. 테스트 5개 신규(1177→1180 passed, 회귀 0). **2라운드(codex 사용한도 소진 — 트랙 폴백 규칙대로 Claude 적대적 리뷰로 대체, 원칙19)**: **치명적** 신규 발견·즉시수정 — `NaverChangeLog.changed_at`이 `server_default=func.now()`인데 SQLite에서 이는 **UTC 반환**(실측: SQLite `CURRENT_TIMESTAMP` 13:xx vs KST 22:xx, 9시간 차)이라, `executed_at=now`는 이미 명시(KST)돼 있었으나 `changed_at`은 누락돼 있었다. guardrail_gate 쿨다운(`now - last_change_at`)·`_build_guardrail_context` 오늘 자정 경계·`resume_candidates` pause_date 계산이 전부 `kst_now()` 기준으로 비교하는데 `changed_at`만 UTC였던 것 — **쿨다운이 상시 "9시간+ 지남"으로 오판정돼 D-NAO-19 안전장치가 사실상 무력화**(fail-open), 일일상한도 00~09시 KST 창에서 경계 오류. X1a의 add_negative_keyword부터 잠재했으나(과거엔 changed_at을 시간계산에 쓰는 코드가 없어 무해) X1b의 guardrail_gate가 최초로 소비하며 실제 결함이 됨. 전 8개 change_log 생성 지점에 `changed_at=now`(executed_at과 동일 KST 값) 명시 추가. 테스트 6개 신규. 중간 지적(medium) 1건은 검토 후 의도적 비대칭으로 판단·주석 보강(resume_candidates가 pause_candidates와 달리 부모체인 미확인 — 과거 정지 근거는 부모상태와 무관해 왜곡 경로 없음, 판단 재검토 필요시 Jino 논의). 테스트 총 1186 passed(1180→1186), 회귀 0.
- [x] X1b T5 D+7/14 채점 배선 확인 — **완료(2026-07-10, Sonnet, 배선확인 중 X1a 원년 결함 치명적 발견·수정)**: proposal_scoreboard.run_daily()가 "미검증 실행"을 `outcome IS NULL`(Phase 6 설계 원안·`test_naver_proposal_scoreboard.py`의 `_change()` 기본값이 이미 그렇게 되어 있었음)로 식별하는데, **X1a T3의 `_execute_add_negative_keyword`가 성공 시 `outcome="executed"`를 즉시 기록**하고 있어 그 필터에 영원히 안 걸림 — **D-NAO-14 학습루프의 핵심(D+7/14 실측 대조)이 X1a 배포(2026-07-10) 이후 단 한 건도 채점되지 않는 상태**였다(negative_keyword든 X1b의 update_bid/set_user_lock이든 전부 동일 결함 상속). 수정: 3개 성공 write 지점(add_negative_keyword·update_bid·set_user_lock)에서 `outcome="executed"` 제거(outcome 미기록, D+14까지 NULL 유지) — "실제 성공"의 판별 기준을 `outcome`에서 `dry_run=False AND after_value IS NOT NULL`로 교체(같은 세션에서 만든 `_detect_external_change`·가드레일 쿨다운 쿼리·`resume_candidates` 3곳도 함께 전환, 전부 `outcome=="executed"`를 참조하고 있었음). 라우터 API 응답의 `outcome` 필드도 이제 실행 직후 `null`이 정상(성공 판정은 HTTP 200 여부로, 프론트는 이 필드를 참조하지 않음 확인). 기존 13개 테스트 수정(라우터·delegation_gate·execution_harness·diagnosis 전반) + 신규 통합테스트 1개(`test_executed_change_log_is_picked_up_by_proposal_scoreboard_after_verify_date` — harness.execute()→proposal_scoreboard.run_daily() 종단 연결 직접 검증, 수정 전이었다면 pending=0으로 영원히 실패했을 테스트). 테스트 총 1187 passed(1186→1187), 회귀 0. **T5가 원래 요구한 "배선 확인"이 곧 이 발견으로 이어짐 — 배선은 이제 실제로 연결됨.**
- [x] X2 T1 response_curve_builder — **완료(2026-07-11, Sonnet, TDD)**: 순수함수 SA. α→(cost,revenue,ROAS) 응답곡선. 입찰탄성 모델 clicks∝α^ε, cost∝α^(1+ε). remaining_fraction(시간대 가중·linear 폴백), fit_elasticity(로그선형 2점+, 클램프 0.1~1.0), build_response_curve(forecast×hourly_pattern×actuals×rpc). 테스트 20개.
- [x] X2 T2 pacing_controller (αB·αC 이분법) — **완료(2026-07-11, Sonnet, TDD)**: 순수함수 SA. find_alpha_budget(선형보간 cost→α), find_alpha_roas(역방향 ROAS≥target), compute_pacing_alpha(min{αB,αC}+binding_constraint 라벨). ALPHA_MIN=0.5, ALPHA_MAX=1.5. 테스트 15개.
- [x] X2 T3 flight_loop 크론 (dry-run 1주 → 실전환) — **완료(2026-07-11, Sonnet, TDD)**: Harness. optimizer='ours' 캠페인별 원료 pre-compute(forecast·hourly_weights·actuals·rpc·remaining_budget)→T1→T2→change_log 기록(action="flight_pacing"). 캠페인별 에러격리. 크론 `15 */2 * * *`(2시간 주기, :15 — snapshot :05 이후 여유). dry_run=True 기본. 테스트 8개. 총 43/43 passed.
- [ ] X3 T1 DHEB 계층 풀링
