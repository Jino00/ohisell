...
경계(불변): 학습은 파라미터만 조정 — 가드레일 상수(±15%·예산상한·BEP하한)와 권한 단계는 학습 대상 아님. 적용 최소 표본 n≥5, 미달 시 기본값. 콘솔에 성공률 추이·보정 곡선 성적표 상시 공개. 구현 시점: 기반(predicted 기록·outcome 판정)=P3, 항목 1~5 환류=P5, 항목 6=P0 산출+월간 잡.
