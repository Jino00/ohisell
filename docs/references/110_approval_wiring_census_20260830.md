...
| **12:20**(배포 후) | 「자동운영 스코프 밖 광고그룹(grp-…) — **D-NAO-244**」 | explore 11 · hourly 5 | `engine_approve`가 **승인 «전»** 거부 ⇒ 죽은 카드 미생성 |
...
⚠️**한 가지 남는 것**: 12:20 발화의 blocked 40건 중 **23건은 스코프가 아니라 「daily 손실상태 제외 — bleeding」**(DL 고삐)이다. 즉 이 시간의 후보 억제에는 두 게이트가 같이 작용했고, 스코프 게이트 단독 효과는 위 16건과 15건의 pending 잔존으로만 잰다.
