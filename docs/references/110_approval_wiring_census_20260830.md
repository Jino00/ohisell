...
2. **답은 「배선 누락」이 아니라 「의도」다 — 단 성격이 셋으로 갈리고, 셋 중 둘에 실질 문제가 있다**: 승격은 **전 기간 800건 중 승인 0건**이고 07-29 이후로는 사람 처분(rejected)마저 끊겨 만료만 쌓이며, 레거시 제외(`negative_keyword`)는 **위임 영구 제외 목록에서 빠져 있다**(지금 안 터지는 이유는 설계가 아니라 런타임 값이 빈 배열이라서다).
...
| 관측 | 값 |
|---|---|
| `search_term_promote` 총 생성 | **800건 (전 기간)** — expired **395**(07-21~08-15) · pending **280**(08-16~08-29) · **rejected 125**(07-21~07-29) |
| 생성 속도 | **하루 20건 균일**(08-18~08-29 12일 전건 20/일) ⚠️아래 시간대 캐벗 |
| 승인된 것 | **0건 — 전 기간** (`approval_source IS NOT NULL` 카운트 0) |
...
★★**여기가 핵심이다**: 승격 후보는 **정보성으로 분류되어 콘솔의 «실행형» 필터(`informational=false`)에서 빠진다.** 그 필터는 D-NAO-47이 *"실행형은 실행형으로 질의한다"*며 만든 것이고(`routers/naver_ad.py:371-381` docstring), 정보성이 실행형을 목록에서 밀어내는 것을 막으려는 설계였다. **그 설계의 부작용으로, 승격 후보는 실행형 목록을 보는 사람 눈에 안 들어온다.**

★★**단 「원리적으로 못 본다」는 과한 말이다 — 실측이 그렇게 말하지 않는다**: `rejected` **125건이 07-21~07-29에 실재**한다. 즉 **초기엔 사람이 그 목록을 보고 처분했다.** 07-29 이후로 rejected가 0이고 08-15까지 expired만 395건 쌓였다. ⇒ 정확한 서술은 **「보이지 않는다」가 아니라 「07-29에 사람 처분이 끊겼고 그 뒤 아무 처분도 안 나온다」**이다. 정보성 분류는 그 상태가 **눈에 안 띄게 지속되도록** 돕는 구조적 요인이지 유일 원인이 아니다. «왜 07-29에 끊겼나»는 이 문서가 답하지 않는다(그 무렵 07-30 전면 정지가 있었다는 것은 시점 일치일 뿐 인과 아님 — §6 [미상]에 남긴다).

⚠️**시간대 캐벗(2026-08-30 실측)**: `naver_proposals.created_at`은 **한 테이블 안에서 UTC와 KST가 섞여 있다** — 엔진 생성 경로는 `server_default CURRENT_TIMESTAMP`(**UTC**)를 그대로 받고(`search_term_promote` 최근 5건 `2026-08-29 23:50:05` = 08-30 08:50 KST · `negative_keyword` `23:00` = 08:00 KST · `bid_up_explore` `02:20:25` = 11:20 KST), 일부 경로만 `kst_now()`를 명시한다. ⇒ 위 「하루 20건」의 **건수는 유효**하고 **달력일 라벨만 9시간 밀린다.** 이 표의 날짜로 다른 KST 축(크론 발화·`scheduler_state`·`naver_change_log`)과 조인하면 하루가 어긋난다 — 창을 거는 분석은 유형별로 시간대를 먼저 확인할 것(memory `sqlite-server-default-now-is-utc`의 «한 테이블 안» 판).

⇒ **395건 만료**는 「사람이 결재를 유보했다」가 아니라 **「07-29 이후 아무 처분도 안 붙은 채 14일 뒤 자동 소멸했다」**이다. 두 문장은 처분이 다르다 — 전자는 정상이고 후자는 **재료가 새는 것**이다(북극성 §4-4 「학습 재료」·D-NAO-251 증거 유실과 같은 결).
...
## §7. ①의 라이브 증거 — 12:20 무인 발화가 새 동작을 확증했다

배포 11:40:42 KST(커밋 `c464f709`, 매니페스트 `02:40:44Z`) 직후 첫 무인 발화를 관측했다.

**무인 판정**: `scheduler_state.run_naver_auto_operator_hourly.last_run_at = 2026-08-30 12:20:25.128753` — 크론 슬롯 `:20`과 소수점 자리까지 직전 발화(`11:20:25.304794`)와 같은 패턴이다(수동 트리거는 임의 시각이라 이 모양이 안 나온다).

★★**사유 문자열이 바뀐 것이 지문이다** — 같은 그룹, 같은 레인인데 «어디서 막혔는가»가 옮겨 갔다:

| 발화 | blocked 사유 원문 | 건수 | 의미 |
|---|---|---|---|
...
**시간대 무관 증거(id 기준)** — 위 캐벗 때문에 시각 창 대신 id로 잰다:

| 관측 | 값 |
|---|---|
| 배포 전 마지막 죽은 카드 | id **6815**(11:20 발화 산물) |
| id 6815 이후 새로 생긴 `approved` | **0건** |
| 12:20 발화가 만든 것 | id 6849~6863 = `bid_up` 5 + `bid_up_explore` 10 = **15건, 전건 `status='pending'`·`approval_source` NULL** |
| 죽은 카드 총계 | **133 불변**(배포 전과 같음. 배포 전 오늘 하루에만 +39였다) |
| 대조군 — 어제 같은 슬롯 | 08-29 11시 **8건** · 12시 **8건** 죽은 카드 생성 |

⇒ 수리가 docstring에 약속한 동작 그대로다: *"스코프 밖이면 pending인 채로 남고 blocked 일기가 남는다(종전엔 approved로 커밋돼 죽은 카드가 됐다)."*

⚠️**한 가지 남는 것**: 12:20 발화의 blocked 40건 중 **23건은 스코프가 아니라 「daily 손실상태 제외 — bleeding」**(DL 고삐)이다. 즉 이 시간의 후보 억제에는 두 게이트가 같이 작용했고, 스코프 게이트 단독 효과는 위 **15건**(explore 10 + hourly 5)과 pending 잔존 **15건**으로 잰다 — ★**두 수가 1:1로 정확히 맞는다**(blocked 1건 ↔ pending으로 남은 제안 1건).

★**콘솔 표면**: 기존 133건은 상태를 바꾸지 않았고(원장에서 「엔진이 승인했다」는 사실과 C7 학습 재료를 지우지 않기 위해), 대신 `real_write_blocker(p, db)`가 전건 `executable=false` + 사유를 돌려준다(prod API 실측 133/133, 18개 그룹). 프론트는 `p.executable`로 실행 버튼을 비활성화하고 사유는 `title` 속성에 넣는다(`NaverAdOptimizationConsole.tsx:903-904`) ⇒ **「실행 가능」 거짓 표시는 해소됐으나 사유는 호버해야 보인다**(소관: PAO UI/UX 트랙).
...
- **승격 후보 800건의 품질** — 「승격했으면 이익이었을까」는 재지 않았다(홀드아웃 없이 집행에 넣지 않는다, 북극성 §7).
...
- **왜 2026-07-29에 사람 처분(rejected 125건)이 끊겼는가** — 07-30 전면 정지와 시점이 겹치나 인과는 확인하지 않았다.


---

## §8. 후속 실측 — 「단일 문」의 사정거리는 `auto_operator` 모듈까지다 (2026-08-30 14:2x KST)

Jino 질문 *"이 기능의 스코프가 전체로 적용되어 있는거 맞아?"* 에 답하며 승인 쓰기 지점을 전수로 셌다. **커밋 `bd8e7572`의 주장 *"다섯 지점 + 콜드 레인이 전부 이 문을 지난다"*는 `auto_operator` 안에서는 참이고, 모듈 밖 승인자 3명은 그 문장의 사정거리 밖이다.**

**`engine_approve` 호출부 전수(7곳)**: `auto_operator.py` 1008·1184·2857·2987·3113·3952 + `cold_start_bid_lane.py:285`.

**문을 «안» 지나는 승인 쓰기 지점(3곳)**:

| 좌표 | 자체 스코프 검사 | 라이브 실적 |
|---|---|---|
| `search_term_ss_lane.py:267` (`_autofire_exclude`, 파워링크 자동발사) | ✅ **있음** — `blocked_by_scope`(377-378) · `_in_scope`(959-980) | `ss_exclude` 1건(07-22 실행) |
| **`delegation_gate.py:187`** (조건부 UPDATE 자동승인) | ❌ **없음**(`blocked_by_scope`·`adgroup_scope` grep 0건) | 승인원 `delegation` **전 기간 0건** · `expert_delegated_types = []` **07-16 이후 불변** ⇒ **휴면** |
| **`probe_revert.py:622`** (탐침 되돌림 `bid_down` 생성·즉시 승인) | ❌ **없음** | 되돌림 대상 = 「실행된 `probe_op`」인데 전 기간 `probe_op` **1건이고 그마저 미실행** ⇒ 대상 0건, **한 번도 돈 적 없음** |

⇒ **오늘 죽은 카드를 만들던 경로는 전부 문 안쪽이었고 그건 막혔다.** 남은 둘은 **구조로 막힌 게 아니라 «재료가 없어서» 안 터진다** — 위임 목록에 유형 한 줄을 넣거나 탐침이 재개되면 살아나고, 그때는 각자 스코프 검사가 없으므로 죽은 카드가 다시 생긴다.

★**이건 그 커밋이 고치려던 병의 재현 조건이다.** 커밋 스스로 *"규칙을 레인마다 다시 적는 방식이 실패했으므로 승인하는 자리 자체를 하나로 만든다"*고 적었는데, **「하나로」의 경계가 모듈이었다.** 같은 저장소 기억(`same-defect-three-times-fix-the-shape`)이 말하는 「모양을 고쳐라」는 **모듈 경계가 아니라 «행위 경계»로 그어야** 성립한다.

⚠️**수치 주의**: 승인원별 집계에서 `explore_op` 미실행 167 · `auto_op_hr` 146 같은 값은 **status 필터 없이** 센 것이라 죽은 카드 수가 아니다. 죽은 카드(=`approved` ∧ `executed_change_log_id IS NULL`)는 **133**이다.

**처분**: 기록만. 고치지 않았다 — 이 세션 n=1의 합격기준엔 「문이 전부를 덮는가」가 없었고 완료 QA가 이미 끝났다(판정 뒤 산출물을 고치면 판정이 낡는다). 문을 모듈 밖까지 넓히는 것은 **별도 슬라이스**이고 대상은 `delegation_gate`·`probe_revert` 2파일 + 회귀 테스트다.
