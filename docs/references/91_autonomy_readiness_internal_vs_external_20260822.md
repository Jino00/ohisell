...
> ⚠️**ref 번호 91은 잠정이다** — 작성 시점에 저장소 접근이 막혀 `ls docs/references/` 로 최댓값을 확인하지 못했다. 파일을 옮길 때 재확인할 것.
...
| # | 항목 | 근거 |
|---|---|---|
| M3-a | **D-NAO-223 명문화** — 채점기 교정을 §4 합격기준의 별도 항목으로. 「M3에 포함」이 아니라 「§4에 문장으로」 | D-NAO-223 · 교훈 #343 재발 방지 |
| M3-b | **M3↔M4 순환의 경계 문언** — 「M3 합격 = 배선 + 소급/표본 증거까지, 새 식의 첫 라이브 채점은 M4 창의 관측 항목」 | Fable §4 · 없으면 M3이 종결 불가 M |
| M3-c | **첫 확인 항목**: 「`dry_run=False` 행이 07-30 이후 0건」이 실제로 참인가 (D-NAO-223 전제의 미실측 절반) | D-NAO-223 각주 |
| M3-d | **§8 미결**: 기존 150건 `outcome`의 소급 재채점 여부. ★`success` 2행(id 974·975)은 `outcome IS NULL` 필터 때문에 **소급 안 하면 영원히 새 식의 사정권 밖** | ref 90 §8-A · 교훈 #274 |
| M3-e | **값 정확도 게이트(E6)** — 성적표가 소비하는 「총이익」 값의 grain·지연·커버리지를 명시. 상품BEP 230/854가 분모에 어떻게 반영되는지 | E6 · F4 |
