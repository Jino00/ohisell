...
- **★전환 액션 유형도 분리 불가 — sentinel conv = 구매 + 장바구니 합**(`/stats`의 `ccnt`는
  purchase·add_to_cart 등 전환 액션 전량 합계). 상세 행(stat-reports AD_CONVERSION)은 구매만
  `conv_*`, 장바구니는 `cart_*`로 나뉘므로 **회계 의미가 다르다**. 계정 전체 실측에서
  `sentinel conv ≡ 상세 conv + 상세 cart`(잔차 0) 확인. 따라서 sentinel 행을 상세 행과 함께
  합산하는 집계는 분자만 이중가산된다 — 집계 SA는 `adgroup_id != '__backfill__'` 필터 필수.
  (2026-07-28 실사고: `cart_conversion_rate`가 이 필터 없이 배포돼 글로벌 전환율 1.675→3.998.)
