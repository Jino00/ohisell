# 이슈2 — PAO: 기능은 있는데 «손»이 없어 못 쓰는 자리 (2026-08-30 KST)

> ⚠️★**두 가지 정정** (Fable 구조 검증 2026-08-30 20:4x KST):
> **①「전수」 표기를 뺐다.** 분모 31개는 **조사기 자신의 열거**이지 기계가 센 전수가 아니다(§7이 한정을 자백한다). 이 저장소 금지선 *「전수 조사 완료라 쓰지 않는다」*에 걸리는 표기였다.
> **②★분류 A~D가 불완전하다.** 손 A~D는 제어 루프의 **뒷단(판단 이후)만** 자른 분류라, 같은 날 이슈3이 낸 두 신종(**교차 채널 «눈» 부재** · **`launch_target_rank`가 NULL이라 무력화된 방어책**)이 **A~D 어디에도 안 들어간다.** 제안된 대안은 **«단계 × 상태» 행렬**이다:
>
> | 단계(제어 루프 순) | A~D 대응 | 빠진 것 |
> |---|---|---|
> | **눈** — 입력·신호 | **없음** | 교차 채널 입력 부재 |
> | **자** — 판정 기준·임계값 | 부분(§4 상수 438) | |
> | **손** — 실행 writer·입구 | A·B | |
> | **표면** — 사람이 봄 | C | |
> | **되돌림** | D | |
> | **학습** — 결과→기준 | **없음**(이슈1 6-1이 대신) | |
>
> 여기에 **상태 축을 직교로**: **없음 / 있으나 값이 빔 / 있으나 꺼짐(dormant) / 있고 삶**. `launch_target_rank` NULL은 「되돌림·방어 × 값이 빔」이다 — A~D에 못 들어간 이유가 **상태 축의 부재**였다. ★이 상태 축은 새 발명이 아니라 이 저장소의 기존 교훈 **[[existence-gate-is-not-maturity-gate]]**의 분류 편입이다.
> **개편은 계약 D-NAO-280 P1로 2026-08-31 실시됨.** 아래 §2 표는 A~D 축 그대로 두되, 이 한계를 알고 읽을 것.
> ⚠️**삼중 계상 해소됨**: 학습 재료 만료 시계 · 마지막 실집행 N일 · `profit_scorecard` 표면 · 밀린 액셀 카운터가 이슈1(ref 114)과 중복 등재돼 있던 것이 해소됐다. **유일 계상은 정본 `docs/PAO_OPS.md` §12 행 5·8·12·14**(참고: 보정계수 하한 재검 주기 = §12 행 7).
...
| 26 | **총이익 스코어카드** | n/a | 크론 08:40 | **일기+Slack만** | n/a | `profit_scorecard.py::run_profit_scorecard` | 화면·API 0 → 정본 §12-12 |
...
10. **총이익 스코어카드가 Slack·일기에만** (단 총이익 «수치» 자체는 스코프 화면에 있음 — 산출 경로가 둘). → 정본 §12-12
11. **원장 무효화 버튼 없음** — API는 사유 필수·감사 보존까지 설계됐는데 화면 호출 0.
12. **캡에 밀린 액셀 카운터 없음** — `proposal_pipeline.py::_apply_gave_priority`가 방어 클래스 무조건 선순위. → 정본 §12-14
13. **「마지막 실집행 이후 N일」 카운터 없음** — 커맨드 센터가 창 기준 「N회」만 보여줘 **32일 침묵이 「이번 주 0회」로만** 읽힌다. → 정본 §12-5
14. **학습 재료 만료 시계 없음** — `wisdom_candidates.py::_HARVEST_LOOKBACK_DAYS`=90 + 마지막 실집행 07-29 ⇒ 🧠 2026-10-27경 창 밖. → 정본 §12-8
