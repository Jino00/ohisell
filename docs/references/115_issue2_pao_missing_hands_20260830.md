...
> **개편은 미실시 — Jino 결정 대기.** 아래 §2 표는 A~D 축 그대로 두되, 이 한계를 알고 읽을 것.
> ⚠️**삼중 계상 시작**: 학습 재료 만료 시계 · 마지막 실집행 N일 · `profit_scorecard` 표면 · 밀린 액셀 카운터가 **이슈1(ref 114)과 중복 등재**돼 있다. 유일 등록부는 정본 `docs/PAO_OPS.md` §12여야 한다는 것이 구조 검증의 판정이다.
