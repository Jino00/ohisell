...
#### 07:00 KST 예약 크론 발동 (MOP 6245/03 D+1 부분) — 06:20 점검과 동일 결론
- 예약 크론(473a33fb류)이 07:00 발동 → `mop_unit_6245_growth_observation.md` D+1 append. **03 07-13 부분(07:00까지)**: 노출 178·클릭 0·지출 0원(이른 아침). **03 소재 입찰가 = D0 baseline과 동일(800~2,400원) → MOP 아직 입찰 미변경**(유닛 오늘 집행 시작, 07:00엔 손 안 댐). MOP 콘솔 뷰는 SESSION_EXPIRE로 캡처 실패(Jino 로그인 필요).
- **04(우리)**: 08:00 크론 전이라 제안 미생성. → 양군 모두 07시엔 변화 0. **진짜 D1 대조=오늘 저녁**(집행 하루치 축적 후).
