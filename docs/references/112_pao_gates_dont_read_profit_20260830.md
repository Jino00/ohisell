...
## §0. ★정정 (2026-08-30 18:5x KST — 초판 발행 25분 뒤, Jino 질문 *"우리 목표가 총이익인데 그걸 안보는게 말이 돼?"*에 답하다 발견)

**초판 §1의 「총이익을 읽지 않는다」는 과했다. 정확히는 「총이익 최대점을 «보수 프록시»로 대신 보고 있고, 정밀하게 볼 부품은 만들어 놓고 안 켰다」이다.**

초판이 놓친 것: **`expansion_allocator.py`에 `marginal_stop`(한계 정지 근사)이 실재하고, 그 주석이 스스로 「D-NAO-59 꼭짓점」이라 부른다.** 초판은 `profit`이라는 **낱말**을 세느라 `marginal` 계열을 통째로 놓쳤다 — `marginal`을 다루는 파일은 **9개**다(`auto_operator` · `bid_step_types` · `budget_allocator` · `expansion_pressure` · `expansion_allocator` · `exploration` · `proposal_pipeline` · `search_term_dim_ingest` · `proposal_writer`).

★**이 정정 자체가 이 문서의 방법론적 교훈이다**: 「어떤 개념을 코드가 다루는가」를 **한 낱말로** 검색하면 반드시 샌다. 목적함수를 찾을 땐 `profit`·`marginal`·`ceiling`·`bep` 계열을 **함께** 세야 한다.

**§1~§6은 초판 그대로 두고**(사실 관계는 대부분 유효하다) **§2-5를 신설해 정정 내용을 싣는다.** 결론이 바뀌는 지점은 §2-5와 §5다.

---

## §1. 한 줄 답 (★§0의 정정을 적용해 읽을 것)

**PAO는 광고 성과 지표를 아주 많이 읽는다. 그런데 «할까 말까»를 가르는 사전 판단 게이트가 읽는 것은 전부 ROAS·BEP 비율·클릭수·전환건수이고, «원화 총이익 절대액»을 계산해 그 크기로 판단을 가르는 사전 게이트는 0개다.**

★단 이것이 「총이익 최대화를 시도하지 않는다」는 뜻은 **아니다**(§0·§2-5) — 설계는 **한계 ROAS = BEP = 총이익 최대점**이라는 정식화 위에 서 있고, 그 꼭짓점 판정이 **근사로** 구현돼 있다.

원화 총이익은 계산도 된다 — 정확하게, 잘 짜인 코드로. 다만 **그 값을 소비하는 곳이 계산한 자기 자신뿐**이다.
...
### 2-5. ★정정의 본체 — 설계는 옳았고, 막힌 것은 «원료»다

**트랙 220행(설계 원문)**:
> *각 상품·환경에서 "총 이익(절대 이익액)"을 최대화한다. ROAS 최대화가 아니다 — **평균 ROAS가 떨어지는 것을 감수하고, 한계(marginal) ROAS가 BEP에 닿을 때까지 순위를 올려 볼륨을 확장**해 총 이익을 키운다. … (수학적으로 동일 꼭짓점: **한계 ROAS=BEP = 총 이익 최대점**)*

이 정식화는 **참이다.** 그러니 ROAS를 게이트로 쓰는 것 자체는 틀리지 않았다 — **한계 ROAS라면.**

**코드가 스스로 자백하는 것** (전부 이 조사에서 직접 읽은 주석 원문):

| 좌표 | 자백 |
|---|---|
| `expansion_allocator.py`(§ 한계 정지 근사 블록) | *"한계 정지 근사(marginal_stop, **D-NAO-59 꼭짓점**) — **★보수 프록시(정밀 한계ROAS 산출 아님)**"* |
| `expansion_allocator.py::allocate_expansion` 조건④ | *"marginal_stop 아님(`own_ratio ≥ EX_MARGINAL_STOP_RATIO`) — **한계 정지 근사가 최종 브레이크**"* |
| `budget_allocator.py` 머리 | *"**D-S3-c 연기 사유**였던 **marginal ROAS 인과추정**…"* |
| `budget_allocator.py` 본문 | *"…사실 관측만 제공 — **marginal ROAS 인과추정 아님**(D-S3-c…)"* |
| `expansion_allocator.py`(프록시 한계 주석) | *"slope 프라이어가 없으면 정지 판정 불가 → `marginal_stop=False`" · "자기 ROAS 미측정(표본 미달)도 marginal_stop 불가 → False"* |

**정밀 한계를 낼 수 있는 부품은 실재한다** — `response_curve_builder.py`(6,001 bytes). 그런데 이 모듈을 import하는 파일은 **`flight_loop.py`와 `pacing_controller.py` 둘뿐**이고, **둘 다 dormant**다(§3-4: `flight_loop`은 `dry_run` 분기 0건의 관측기 · `pacing_controller`의 α는 로그 기록만).

### ★닫힌 고리 (🧠 — 인과 해석이지 실측이 아니다)

```
실집행 0건(07-30~)  →  인과 추정 불가  →  정밀 한계 ROAS 산출 불가
   →  보수 프록시(평균 ROAS 비율)로 대체  →  프록시가 보수적 ⇒ 확장 억제
   →  실집행이 더 안 남  →  ┘
```

「한계 ROAS」는 **「입찰을 올리면 매출이 얼마나 늘어나는가」**라서 인과 추정이 필요하고, 인과 추정에는 **실제로 밀어 본 데이터**가 필요하다. 그 데이터가 없으니 프록시로 대체됐고, **프록시가 보수적이면 항상 브레이크 쪽으로 기운다** — D-NAO-85 실측(ROAS +7% · 매출 −52%)과 같은 방향이다.

★**그래서 정확한 진단은 「총이익을 안 본다」가 아니라 「임시 프록시가 고착됐다」이다.** 주석은 임시라고 적어 뒀는데(「보수 프록시」·「D-S3-c 연기」) 한 달 넘게 그대로다.
...
★**북극성 §7이 적은 「액셀·브레이크가 대칭인가」보다 한 층 아래에 이 문제가 있다** — 개수를 맞춰도 **둘 다 «평균» ROAS를 자로 쓰면 꼭짓점에 못 닿는다**.

### ★§0 정정 반영 — 1.25는 근거 없는 상수가 아니라 «프록시»다

초판은 1.25를 「근거 없는 문턱」처럼 읽었으나, §2-5가 밝힌 대로 **`EX_MARGINAL_STOP_RATIO`·`EX_PRESSURE_RATIO`는 한계 정지 꼭짓점의 «보수 프록시»**다. 즉 설계 의도는 「평균 ROAS가 이 정도는 돼야 한계 ROAS가 아직 BEP 위일 것」이라는 **대리 추론**이다.

⇒ 그러므로 물어야 할 것은 **「1.25가 왜 1.25인가」가 아니라 「이 프록시가 실제 꼭짓점을 얼마나 잘 맞히는가」**이고, 그건 **`response_curve_builder`를 켜서 정밀 한계와 대조해야만** 답할 수 있다. 지금은 대조 대상이 dormant라 **검증 자체가 불가능**하다.

★그리고 프록시의 오차 방향이 문제다 — 보수 프록시는 **꼭짓점보다 «일찍» 멈춘다.** 일찍 멈추면 D-NAO-59가 지목한 「ROAS는 떨어져도 총이익이 느는 구간」을 남기고 나오게 된다. **오차가 랜덤이 아니라 한쪽으로 쏠린다**는 것이 이 구조의 핵심 위험이다 🧠.
