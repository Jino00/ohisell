...
| **15** | `auto_operator.py:3085` → `_servo_economic_ceiling`(`:1494-1517`, 특히 `:1515 rpc_corrected = rpc_raw × cf`) | 시간당 순위 서보의 경제성 상한 → `rank_servo.decide_servo_step(economic_ceiling=…)` → `rank_servo.py:123 clamped = min(raw_target, servo_cap, economic_ceiling, _MAX_BID)` → **실제 입찰 쓰기**. 방향(up)은 `_judge_hourly` 밴드 관제가 이미 정했고 **계수와 무관** ⇒ 여기서 계수가 정하는 것은 오직 «얼마나 올리는가» | **액셀 크기** | **하한** — 「실쓰기 크기만 하한」의 정의에 정확히 해당 |

⚠️★**#15는 이 §1-B 정정판 자신도 처음엔 놓쳤고, 적대 리뷰(PR #345 1R P1-2)가 잡았다.**
초판은 「9파일 전수」를 정독으로 주장했고, 정정판은 grep을 돌렸는데도 `auto_operator`의 **세 번째**
소비처를 빠뜨렸다(`:433`·`:2333`만 등재). 같은 파일 안에 소비처가 셋인데 둘만 세는 실패다.
**실해**: 라이브 계수 1.3133·target_roas 1.9358·pooled RPC 2,000원 기준 서보 상한이
**1,030원(하한) vs 1,350원(상한)** — 상한을 쓰면 하한이 정당화하는 값보다 **31% 높은 입찰까지**
실제로 올라간다. 서보는 매시 :20 KST에 도는 **실입찰** 레인이다(`scheduler_service.py:993-1001`).
⇒ **전수 = 13개 파일 · 25행**. 초판 §2의 대칭 판정 숫자는 진단 보드 볼륨으로 센 것이라 이
정정에 영향받지 않는다(10~15는 전부 [미실측] 계열).

★**교훈(#348 후보)**: 「전수」를 주장할 때 세어야 하는 것은 **파일이 아니라 호출부**다. 초판은
파일 목록을 안 세서 4파일을 놓쳤고, 정정판은 파일은 셌는데 **한 파일 안의 호출부**를 안 세서
1곳을 놓쳤다. 같은 병의 두 눈금이다 — 북극성 §7 「전수 조사 완료라고 쓰지 않는다」의 실증.
...
| **15 `auto_operator._servo_economic_ceiling`** | 실쓰기 | **하한** | (누락) | 시간당 실입찰 «크기» — 방향은 밴드 관제가 이미 정한다. 상한 사용 시 +31% 과대(1,030 vs 1,350원) |
