...
## 또 다른 사례 — 화면이 헬퍼에 «닿는지» (2026-08-12, D-NAO-176, 교훈 #284)

같은 모양이 프론트에도 있었다: `bucketKeysToRender()`를 순수 함수로 6/6 테스트했지만
화면이 실제로 그 헬퍼를 부르는지는 아무 테스트도 지키지 않아 2R 변이 M2(호출부를
옛 `BUCKET_ORDER` 순회로 되돌림)가 살아남았다 — 「쓰기가 옥션에 닿는가」와 같은 질문을
「화면이 헬퍼에 닿는가」로 반복한 사례. jsdom 렌더 테스트로 결합 층을 직접 묶어 처분.
