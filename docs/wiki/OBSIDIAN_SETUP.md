...
### ✅ 결정: **별도 볼트로 유지한다** (D-NAO-167, Jino 2026-08-10 *"별도 볼트로 유지하자"*)

근거:
- **정본이 다르다** — AIOffice는 KG(SQLite+ChromaDB)가 정본이고 markdown은 **자동 미러**다.
  여기는 **git이 정본**이고 사람·세션이 직접 쓴다. 한 볼트에 섞으면 자동 재생성이 수동 파일을
  덮거나 그 반대다 — 「정본 이원화 금지」 금지선이 바로 이 경우를 막으려는 것이다.
- 옵시디언은 볼트 전환이 한 클릭이라 분리 비용이 작다.
- AIOffice의 KG가 이 위키를 흡수하는 길은 **폐기가 아니라 보류** — 필요해지면 재심한다.

> **이 분기를 다시 열지 말 것.** 재심하려면 트랙에 새 D-N으로 올린다.
