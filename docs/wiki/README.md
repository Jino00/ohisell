...
이 repo 루트를 볼트로 연다 — **단 `scripts/setup_obsidian.sh`를 먼저 돌린다**(열고 나서
제외하면 이미 늦다: 파일 163,106개·워크트리 md 7,000개). 상세는 `docs/wiki/OBSIDIAN_SETUP.md`.

**옵시디언은 뷰이지 쓰기 주체가 아니다** — 사람이 그래프·백링크로 보는 창이고,
갱신은 세션이 git에 한다.

★**AI Office 볼트(`AI Program/Vault/AIOffice`)와는 별도로 유지한다**(D-NAO-167, Jino 결정).
저쪽은 KG가 정본인 **자동 미러**(비즈니스 CRM), 여기는 **git이 정본**인 수동 지혜층이다.
섞으면 자동 재생성이 수동 파일을 덮는다.
