...
## 🟢 Active
- [Wing 세션 자동화](tracks/active/track_wing-session-automation.md) — ⚠️**구조 설계 스캐폴딩(코딩 전, 0/N)**. wing.coupang.com 내부 API를 세션 만료 없이 자동 호출하는 공용 헤드풀 페처 → (A)매출 자동대조(vendor-summary, ref 18) + (B)RG정산 자동수집(S6-auto) 동시 해결. 광고 페처 패턴 복제. **다음 세션: 구조 승인 → Opus 계획 → S1.**
