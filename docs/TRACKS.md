...
## 🟢 Active
- [쿠팡 RG 재고·발송 관제 (Replenishment)](tracks/active/track_coupang-rg-replenishment.md) — RG 현재고+일판매속도+입고 리드타임으로 "언제·몇 개 발송" 역산. 목표 FC재고 2~3일치. 5/7 (S1 입고동기화·S2 리드타임추정·S3 일판매속도추정[평일/주말/휴일 신뢰도게이트]·S4 발송역산[요일인지 forward투영·4-status]·S5 rg_replenishment Harness[배치역산·등가성 784/784 라이브 PASS·GET /replenishment-plan] 완료+codex pass+prod 라이브검증 성공). 다음 S6 UI 컬럼(로켓그로스 탭). D-14 수정(입고 Wing 내부 API 연결).
