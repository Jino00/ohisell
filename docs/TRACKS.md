...
## 🟢 Active
- [쿠팡 매출·광고 정합성](tracks/active/track_coupang-revenue-ad-reconciliation.md) — **7/7 코드 완료**. 종합조망 매출·광고·순이익을 쿠팡 대시보드와 일치 + 계정별 분리. **S1~S7 전부 완료·prod 배포·라이브 검증**(2026-06-14: 계정분리·2중계상수정·RG매출편입[52%갭해소]·net_profit D-3·신선도 reconcile+이중차감 D-12·검산 대시보드·**S5 비-PA 전체(ALL)전환 D-15+커버리지30일 D-13**). RG 신선도=gross 불변 피드로 문서화 종료(D-11). 비-PA=ALL_DELIVERED−DELIVERED, 계정식별 게이트로 net_profit 차감(codex 2R pass). **트랙 핵심 목표 전부 해소 — 운영 단계.** 후속(선택): 옵션×일별 30일 확대(현 7일)·자동대조(봇차단).
