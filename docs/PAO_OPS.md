...
| 레버 | 왜 안 쏘나 | 근거 |
|---|---|---|
| 검색어 제외 | **쇼핑이라 구조적으로 Confirm 전용** | `search_term_ss_lane.py:891` *"쇼핑은 파워링크처럼 자동 발사는 하지 않는다"* — 오늘 08:50 레인이 만든 후보 **11건 전부 `pending`·`approval_source=null`** |
...
| | 무엇 | 왜 지금 |
|---|---|---|
| **A** | **카나리에서 무엇으로 실집행을 만들 것인가** | ①쇼핑 검색어 제외를 콘솔에서 Jino가 Confirm(지금 11건 대기) ②카나리를 파워링크 캠페인으로 바꿔 자동 발사를 관측 ③C2 합격기준을 「Confirm 경유 실집행」으로 개정(=계약 개정) ④그대로 두고 「구조상 실집행 0」을 M4 판정으로 기록. **넷 다 계약 사안이다** |
...
| 부채 | 값 | 소관 |
|---|---|---|
| 죽은 카드(승인됐으나 스코프에 막혀 영구 미실행) | **141건** (08-30 08:5x, 변화 없음) | 이 트랙 — 수리는 main에 있고 배포만 남음 |
...
## §6. 돈 (30일, 관측 2026-08-30)

| | 비용 | 전환 | 전환액 | ROAS |
|---|---:|---:|---:|---:|
| 계정 전체 30일 | 19,923,726원 | 2,225 | 35,377,700원 | **177.5%** |
| **카나리 캠페인 30일** | **7,724,101원** | — | 11,583,480원 | **149.9%** |
| 계정 7일 | 4,051,618원 | — | — | — |
| 어제(08-29) | 537,105원 | — | — | — |
