...
- ~~`tools/rocket_settlement_item_backfill.py`에 커밋 안 된 로컬 diff~~ → **커밋 완료**(`626af28`).
  커밋본 주석이 "표는 인덱스로 고정한다"였는데 코드는 헤더 매칭이라 **주석이 코드와 어긋나 있었다** —
  코드가 맞고 주석이 틀렸다. 함께 차감 계산서 분류도 넣었다.
- **SQLite 파일 3종이 git에 추적되고 있었다** → PR #230으로 분리(`ohisell.db.bak_pre_s1` 22.3MB ·
  `ohisell.db-shm` · `-wal`). `.gitignore`에 "커밋 금지" 의도는 있었는데 글롭이 빗나갔다
  (`.bak.`(점) vs 실물 `bak_`(언더스코어), `*.db`가 `.db-shm`을 못 잡음). 이번 세션에 실제로
  로컬 마이그 테스트 중 지워진 사이드카가 무관한 커밋에 삭제로 섞여 들어가 되돌린 일이 있었다.
  ⚠️과거 히스토리의 22MB는 남는다 — 히스토리 재작성은 병행 세션이 많아 위험이 커서 하지 않았다

## 6. 다음에 할 작업 (미완료)

### ★최우선 — 미수금 연령(aging) 재판정 (Jino 지시 2026-08-06 밤)
전 세션 §6에서 **"실입고일이 없어 못 낸다"**고 막혀 있던 항목이다. 이제 재료가 다 있다.

**왜 필요한가**: 미수금 8,033,970원(ref 45 §12·§18, 159라인)은 금액은 검증됐지만 **"며칠 된
미수인가"를 셀 기준이 없었다.** 청구 우선순위·시효 판단의 근거가 없는 상태다. 이번에 생긴
`receiving_finished_at`(PO)과 `received_at`(계산서 라인)이 그 기준선이다.

**쓸 수 있는 재료(이번 세션에 다 채워짐)**
- `coupang_rocket_purchase_order.receiving_finished_at` — 실입고 완료 시각. 442건(2026-05-09~08-06)
- `coupang_rocket_settlement_item.received_at` — 계산서 라인의 입고일. 9,926행(2025-07-23~08-05)
- 미수 = 발송·입고했는데 계산서 라인에 안 잡힌 몫. **라인이 SKU 그레인이라 이제 라인 대 라인으로 뺄 수 있다**(전엔 PO 헤더끼리만 비교 가능했다)

**착수 전 확인할 것(추정 금지)**
1. ref 45 §12·§18의 159라인이 **지금도 미정산인지 재확인**. 이번 백필로 계산서 라인이 다 들어왔으니
   그때와 판정이 달라졌을 수 있다. 달라졌으면 그 사실부터 보고할 것
2. 연령의 **기준일을 무엇으로 잡을지** 정하고 근거를 댈 것 — 실입고 완료일이 유력하나,
   발송일·거래명세서확인일도 후보다. 임의로 고르지 말고 뭘 골랐는지 밝힐 것
3. **90일 창 밖 옛 PO는 실입고일이 비어 있다**(2,563건 중 442건만). 그 구간 미수는 연령을
   못 낸다 — 0이나 추정으로 접지 말고 "연령 판정불가"로 분리해 셀 것(원칙22)
4. 별도 유형 하나 더 있음: **계산서가 아예 없는 입고완결 PO 31건 434,960원**(2026-08-06 실측,
   전부 08-04 입고라 그땐 "발행 대기"로 판정했다). 이제 연령으로 재판정 가능

**하지 말 것**: 미수금 **금액**을 새로 만들지 말 것. 이번 과제는 이미 검증된 금액에 **연령 축을
붙이는 것**이다. 금액이 달라지면 그건 발견이므로 보고하고 멈출 것.
...
- [ ] 계산서 라인이 SKU 그레인이라 **원가 결합 경로가 열림** — 라인 단위 원가 매칭으로 정합도를 더 올릴 수 있는지 검토
- [ ] PR #228·#230 병합 여부 확인(둘 다 MERGEABLE 상태로 남겨 둠)

## 7. 새 세션 시작 프롬프트
```
.claude/memory/HANDOFF_settlement-axis-to-receiving-date_20260806.md 읽고
§6 최우선 항목(미수금 연령 재판정)부터 진행해줘
```
