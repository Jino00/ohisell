...
> 저장일시: 2026-08-06 (KST, 최초 저장) · **2026-08-06 밤 갱신**(통합 손익 대조원장분 추가, D-NAO-158) · **2026-08-06 밤 재갱신**(기간 탭 오늘 광고비 + 반품 배송손익, D-NAO-159·160) · **2026-08-07 06:3x 재갱신**(수수료 추정=실측 오차 0원 재측정·정정, D-NAO-161, ref 49) · **2026-08-10 재갱신**(`ambiguous` 판정 첫 실발동, 매핑 5건 정정, 교훈 #197, ref 53 — 문서만, 배포 없음) · 트랙: 네이버 SA 광고 최적화 (이 세션은 손익 정합 성격 — 광고 집행 축 아님)
...
### 2-d. (2026-08-10, 새 워크트리 `costmap-20260810`) `ambiguous` 판정 첫 실발동 — 매핑 5건 정정
Jino가 라이브에서 발견해 지시한 매핑 정정. **코드 변경·배포 없음** — 전부
`/api/products/...` API 경유(이중매핑 가드 `_active_option_clash` + 과거 주문 소급연결
`_relink_after_mapping`이 API에만 붙어 있어 직접 SQL로 하면 이미 판 것은 계속 미상으로
남는다). 상세는 `docs/references/53_costmap_ambiguous_first_trigger_20260810.md`.

- **매핑 변경 5건**(옵션ID → 조치 → 매핑ID → 결과):
  - `11619806388`(아이폰12 프로 맥스) — `OHI-0756` 오링크 비활성화(매핑ID 2324) →
    `OHI-0755`(3,500원)만 남음
  - `13676281943`(아이폰12-12프로 공용) — `OHI-0756` 연결 생성(매핑ID 2903) →
    원가 3,500원, 소급연결 2건
  - `13702258112`(갤럭시Z플립8 외부액정3매) — `OHI-0902` 연결 생성(매핑ID 2904) →
    원가 1,645원, 소급연결 1건
  - `11734926172`(갤럭시탭S8) — `OHI-0062`(auto_sync, S7/S8플러스 오매핑) 비활성화(매핑ID 302)
    → `OHI-0061` 4,352.7원 확정
  - `11734926167`(갤럭시탭S10FE) — `OHI-0076`(auto_sync, S8/S9/S10울트라 오매핑) 비활성화
    (매핑ID 348) → `OHI-0075` 4,352.7원 확정
- **★뒤의 둘이 `ambiguous` 판정(D-NAO-157)의 첫 실제 발동** — 채널옵션 하나에 활성 매핑이
  둘이고 원가가 갈린 실사례. 둘 다 원인은 **`auto_sync`(자동 매핑)의 기종 오판**이었고
  사람이 넣은 `excel_master` 쪽이 정답이었다(교훈 #117 실증 사례 추가).
- **라이브 결과(배포 없음, 데이터만)**: 어제(08-09) 미상 2→0개, 원가 414,909→421,082(+6,173),
  이익 263,544→257,372(이익률 14.05→13.72%). 30일(07-12~08-10) 미상 2→0개(매출 101,182→0),
  원가 9,366,004→9,393,703(+27,699), 이익 2,850,167→2,822,468(이익률 6.85→6.79%), 원가
  확인 상품 303→305개. **매출·수수료·광고비·물류비 전부 불변, 기존 상품 중 원가가 바뀐 것
  0건** — 채워진 것은 이 두 상품뿐.
- **교훈 #197**(`.claude/memory/LESSONS_LEARNED.md`): 종전 코드 `min(costed, key=lambda x: x[1])`가
  `product_master.id` 기준으로 골라 이 두 건은 **우연히 id가 작은 정답 쪽을 뽑았던 것** —
  "지금 값이 맞다"가 "판정 로직이 옳다"를 뜻하지 않는다. 매핑 후보 판별의 최강 단서는
  「형제 옵션이 어느 마스터에 붙어 있나」.
...
- **활성 매핑 2,766개 중 127개가 cost_price=0 — 2026-08-10 매핑 정정 이후에도 그대로 127개
  잔존**(오늘 고친 5건은 이 127개와 겹치지 않는다 — 오늘 건 중 `ambiguous` 2건은 이미
  cost_price>0인 오매핑이었고, 나머지 3건도 이 목록 밖). 팔리면 「—」로 뜬다. Jino 원가
  입력 대기 계속.
- **원가 미상 6개 상품 — `13676281943`(아이폰12-12프로 공용) 1건은 2026-08-10 매핑 정정으로
  해소됨(§2-d, ref 53). 남은 5개(매핑 4·원가입력 1) — Jino 조치 대기**:
...
- [ ] **★최우선 — `coupang_1p` 보존법칙 복구**: `coupang_1p/cost`·`coupang_1p/net_profit`이
      ...
      축의 다음 단계.
- [ ] **Jino 데이터 입력 대기**: 원가 미상 5개 상품(매핑 4·원가 1, 2026-08-10 1건 해소·§2-d) +
      cost_price=0인 127개 매핑(2026-08-10 정정 이후에도 그대로 잔존) 중 실제로 팔리는 것부터.
