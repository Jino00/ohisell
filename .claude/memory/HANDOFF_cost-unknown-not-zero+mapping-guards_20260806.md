...
> 저장일시: 2026-08-06 (KST, 최초 저장) · **2026-08-06 밤 갱신**(통합 손익 대조원장분 추가, D-NAO-158) · **2026-08-06 밤 재갱신**(기간 탭 오늘 광고비 + 반품 배송손익, D-NAO-159·160) · 트랙: 네이버 SA 광고 최적화 (이 세션은 손익 정합 성격 — 광고 집행 축 아님)
...
- **★신규 이월(같은 조사) 수수료 추정 비중**: 실측 비율 30일 73.4% / 15일 52.3% /
  **7일 24.3%** — 조회 기간이 짧을수록 (실측이 아닌) 추정으로 채워지는 수수료 비중이
  커진다. 이전 세션(HANDOFF_smartstore-settlement-parity+vat_20260804)에서 예상↔실측
  오차 0원이 확인된 바 있으나, 그건 특정 시점의 검증이고 최근 짧은 구간은 상대적으로
  덜 검증된 상태라는 뜻이다. Jino에게 이미 보고함, 이번 세션에서 코드 변경은 하지 않음.
...
- [ ] **수수료 추정 비중 재점검**: 7일 창은 추정 비중 24.3%로 상대적으로 검증이 얕다. 필요
      시 짧은 창의 실측 커버리지를 별도로 확인.
