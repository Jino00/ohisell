...
- **학습밴드 스코프 불일치**: 09:03 학습 잡 `run_probe_learning(db)`은 `campaign_id`를 안 넘겨 **계정 전체** 집계를 승격·일기에 기록하는데, 실제 게이트는 **캠페인별**로 재계산한다. 오늘 계정 전체는 `1.0-2.0`이 승격돼 있지만 **ours 6개 중 그 밴드를 소비하는 캠페인은 0개**다. 스코프 통일(또는 계정 밴드 폴백) 여부는 **Jino 판단 대기** — 폴백을 쓰면 학습밴드 None인 3개 캠페인의 탐침 하한이 2.5→2.0으로 내려가 라이브 동작이 바뀐다.
...
4. **Jino 판단 대기 3건**:
   - ① 학습밴드 스코프 통일 여부(계정 전체 vs 캠페인별, §3).
