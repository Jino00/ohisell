...
## 22. pause 직후 관측 함정 — 로컬 entity status는 다음 아침 sync까지 stale (2026-07-20)
...
## 23. 설계 시 기존 테이블 전수 확인 — "영속 안 됨" 단정이 신규 마이그레이션을 낳음 (2026-07-20)
...
## 24. 라우팅 우회 — "내가 하는 게 빠르다"로 Fable이 Sonnet 몫을 직접 수행 (2026-07-21)
...
## 25. HANDOFF의 "예약됨" 주장이 실제 스케줄에 없었음 — 예약·자동화 주장도 실측 대상 (2026-07-22)
...
## 26. 회귀 테스트가 프로덕션에 없는 문자열을 검증 — "허구 픽스처" 함정 (2026-07-22)
...
## 27. 레버 오독 — 그룹 bid_amt만 보고 "방치" 단정 (2026-07-22)
...
## 28. 데이터 부재 ≠ 0 — 확정 테이블에 오늘 행이 없는 것을 "노출 0"으로 오독 (2026-07-22)
...
## 29. 순위 중심 보고 — 입찰가 나열은 보고가 아니다 (2026-07-22, Jino 질책)
...
## 30. fail-open 하니스는 SA 간 "빈 상태" 가드가 대칭이어야 한다 (2026-07-23, codex 소급 리뷰 BM P1-2)
...
## 31. prod DB 시간 규약이 테이블마다 다르다 — 감사 쿼리 오독
...
## 32. 크론 "ok" ≠ 레인 작동 — 게이트가 조용히 전건 차단할 수 있다
...
## 33. `dry_run=0` change_log 행 ≠ 실쓰기 — 차단된 제안도 행을 남긴다 (2026-07-27, 04 감사)
...
## 33. wing 3P GMV=0 "모순" — 파싱 버그가 아니라 계정 커버리지 차이
...
## 35. 병렬 세션 supersede — PR 병합 전 fetch가 783줄 중복 배포를 막았다

### 🐛 이슈
wing 페처 fetch-error 보고 스프린트(codex 5R 완주, PR #109)를 병합하려는 순간,
병렬 세션의 PR #106(claim-retry-lease)이 10분 전 같은 두 파일을 더 크게 고쳐
병합돼 있었다. 우리 6개 수정 중 3개는 main이 이미 더 잘 해결(무주기 신호는
성공 채널 신설이 아니라 "요청 소멸+프론트 조기종료"가 정답이었고, rc 3도
RC_LOGIN_REQUIRED로 선점). 기계적 rebase였으면 lease 계약을 구멍 냈다.

### ✅ 해결
병합 불가 → STOP → main 대비 갭 분석(6개 관심사 × covered/not) → #109는
superseded 클로즈(선례 #88) → 잔여 3건+분석 중 발견한 main 신규 버그 1건
(보고 호출 account_key 누락=WING2 보고 유실)만 main 기준 재구현 → PR #111.
783줄 → 110줄(+테스트)로 수렴, 가드 증명(소스 되돌리면 15/25 실패)으로 검증.

### 📌 교훈
① 병합 게이트에서 충돌이 나면 rebase가 아니라 "supersede 여부 판정"이 먼저다 —
   상대 PR 본문을 읽고 관심사 단위 갭 분석부터.
② 오래 걸린 스프린트일수록 병합 직전 fetch 필수 — 완주한 codex 5R도 낡을 수 있다.
③ 신호 채널의 의미는 payload 텍스트로 못 뒤집는다(R3→R4 교훈). 단 "성공 채널
   신설"이 유일해는 아니었다 — 요청 자체를 소멸시키는 계약이 더 나은 해였다.
