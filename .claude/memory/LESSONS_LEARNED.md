...
## 18. `dry_run=0` change_log 행 ≠ 실쓰기 — 차단된 제안도 행을 남긴다 (2026-07-27, 04 감사)
...
같은 날 발견한 자매 함정: `naver_proposals.created_at`이 **KST(앱 명시, 마이크로초 있음)와 UTC(DB 기본값, 마이크로초 없음) 혼재** — 한 테이블 안에서도 규약이 갈린다(LESSON 16 확장). `date(created_at)='오늘'`은 파이프라인 생성분을 통째로 놓친다.
