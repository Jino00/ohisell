...
## 10. 배너 문자열은 진단이 아니다 — "쿠키 만료" 13일 방치 사건

### 🐛 이슈
쿠팡 광고비 수집이 13일(07-04~07-17) 멈췄는데, 화면 배너 "쿠키 만료(재설정 필요)"를 여러 세션이 그대로 옮겨 적기만 하고 아무도 검증하지 않았다. 실제 원인은 쿠키가 아니라 **prod의 request_ad_cost_refresh 크론이 disabled**였던 것. 게다가 toggle API는 미등록 잡을 enable하면 예외를 삼키고 DB 플래그만 바꿔서, 누군가 켰더라도 실제로는 안 돌았을 함정이 겹쳐 있었다. 사용자가 외부 크롤링 스킬 구매(149,000원)까지 검토하게 만들었다 — 봇 차단 문제가 아니었는데.

### ✅ 해결
크론 재활성+재시작으로 즉시 복구(페처가 확정치 12일치 자동 백필 — 데이터 영구손실 없음). 재발 방지 3종: toggle 라이브 등록(job_func_for 단일 매핑), cookie_status에 refresh_cron_enabled 노출→배너가 크론꺼짐 우선 진단, COUPANG_OHITECH_AD 워치독 편입. PR #28.

### 📌 교훈
①UI가 보여주는 실패 사유는 그 UI가 아는 것만 반영한다 — 배너/에러 문자열을 원인으로 옮겨 적기 전에 그 판정 로직이 무엇을 못 보는지 확인하라(이번엔 크론 상태를 못 봤다). ②"별건"으로 분류된 빨간 배너가 13일 산 것 자체가 신호 — 지속 실패는 별건이어도 하루 안에 원인 규명까지는 해야 한다. ③toggle류 API에서 except:pass는 "켰는데 안 켜짐"이라는 최악의 침묵을 만든다 — 상태 변경 API는 실제 반영 여부를 응답에 담아라(live_registered).
