...
## 12. ★세션 말미 추가 — 다음 작업이 정해졌다: 적응형 안전 봉투

**계약서: `docs/PLAN_adaptive-guardrail-envelope.md` (방향 승인 완료, 실행은 새 세션)**

Jino 원문(방향): *"안전봉투도 절대규칙이 아니라 조절할 수 있는 기준으로 만들자 … 기준은 정하데 기준도 상황에 따라서 바뀔 수 있는거지"* → 내 추천안 승인 *"그렇게 진행해줘"* → *"그리고 실행은 새로운 세션에서 진행하자"*.

**한 줄**: 봉투를 하드코딩 상수 → 근거 있는 기준으로. **조이기는 자동, 풀기는 제안 → Jino 클릭 1회.**

**새 세션이 먼저 알아야 할 실측 3건** (상세는 PLAN §2):
- 봉투가 실제로 문 건 전체 차단의 **2.2%**(86/3,863) — 병목이 아니다. 1위는 **소급채점 stale 55%**(지금은 정상, 감시 표면 없음).
- **`±15%`는 「가드」가 아니라 「스텝 정책」**이다(차단 기록 0건) — 더 크게 가려는 걸 자르는 게 아니라 한 회차 이동폭의 정의. 봉투는 **상한**과 **속도**로 성격이 갈린다.
- 손실 확인된 상품도 하루 **−39%**밖에 못 내려간다 ← 조이기 자동화의 근거.

**착수 전 확인된 전제**: 소급채점 이력 **33일치** 보존(3일 연속 판정 가능) · 정지 상태에선 **시간당 레인이 캠페인 루프에 진입조차 안 한다**(합격기준을 레인 로그로 못 잡음 → 읽기 전용 현황판으로) · 파라미터 저장소 `NaverAccountSettings` KV **이미 존재**(마이그레이션 0).

**P1부터 착수 권장** — 파라미터 DB층 + 폴백 + 봉투 현황판 + 소급채점 stale 배너. P1만으로도 가시성과 원복 스위치가 생긴다.

⚠️ **§1 계약 승인은 아직 안 받았다.** Jino가 승인한 것은 **방향**이고, PLAN 문서의 5요소(목표/안함/판단기준/금지선/합격기준/예산)에 대한 **명시적 승인 1회**를 새 세션이 받고 착수할 것.
