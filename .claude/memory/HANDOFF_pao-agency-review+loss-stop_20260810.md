...
- **★단, 같은 잣대로 우리를 재면 우리가 더 나빴다** — 우리 자동운영 기간(07-21~29) **PAO 6개 캠페인 = 계정 광고비 57.7% 쓰며 −726,654원 적자** vs **대행사 나머지 42.3%로 +118,053원 흑자**. 어제 낸 「정지 전 계정 −608,601원 적자」의 정체는 대부분 우리 것이었다.
  - ⚠️**재귀속 주석(2026-08-10, D-NAO-164)**: 이 «우리 기간» 수치를 «PAO의 성과»로 단정하지 말 것 — 적어도 03 캠페인은 소재 36/36이 `use_group_bid_amt=0`(실효 입찰=소재)인데 PAO는 그룹 입찰만 59건(전부 상향) 썼고, **바인딩 입찰은 그 기간에도 대행사가 쥐고 있었다**(07-24 소재입찰 2,610 설정 실측). 다른 5개 캠페인의 실효 레이어는 미실측.
...
- [ ] **B-1 `change_log` 쓰기 가드** — `OURS_ACTIONS_BY_AXIS` 밖 action으로 `dry_run=False` 행을 쓰거나 `axis_value`가 못 읽는 값이면 경고/거부(교훈 #196. 지금은 **규칙을 아는 사람만** 지킬 수 있다)
  - **★시각 규약도 가드에 포함** (Jino 2026-08-10: *"무조건 KST로 사용하는거지"*): `changed_at`은 **KST naive**다(자동 행이 그렇고 `_executed_bid_ups_today`가 그 전제로 「오늘 0시」를 센다). 2026-08-10 수기 정지 2행이 네이버 `editTm`의 **Z(UTC) 시각을 그대로 넣어** 9시간 이르게 남았다(교정 완료, 원장 5854·5855). 가드는 `changed_at`이 **KST 오늘 기준 미래/과도한 과거면 거부**하거나, 최소한 UTC 원문(`...Z`)을 그대로 받는 경로를 막아야 한다. 관련 위키 패턴 [[read-external-values-before-writing]].
- [ ] **B-2 집계 정본 헬퍼** — sentinel/실단위 택일 + 검산식을 공용 함수로(교훈 #195. 서비스 코드는 각자 제외하지만 **조회·분석 경로엔 가드가 없다**)
- [ ] **B-3 BEP 기준선 표면화** — 판정에 어느 계수를 쓰는지 화면·로그에
- [ ] **B-4 ★쓰기 대상 = 실효 입찰 레이어** (D-NAO-164, 2026-08-10) — `use_group_bid_amt=0`이면 소재 입찰에 쓴다(쓰기 경로는 D-NAO-75 GET 전체객체 PUT 재사용). 03 캠페인에서 PAO 집행 59건 전부가 그룹 레이어라 **옥션에 안 닿았다**(교훈 #202). 착수 시 전 캠페인 `use_group_bid` 분포 실측 선행. **P0-a의 거부권도 실효 레이어에 걸어야 의미가 있다** — B-4 없이 P0-a만 하면 거부권이 무대 연기를 막는 꼴.
