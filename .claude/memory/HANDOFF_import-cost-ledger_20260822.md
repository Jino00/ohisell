...
## 세션 후반 (18:1x~20:4x) — 8개월 전수 실측과 중복 발견

★**정본은 ref 92** — `docs/references/92_import_ledger_8months_census_20260822.md`(데이터에서
생성됐다, 손으로 옮긴 숫자가 아니다). 이 절과 어긋나면 ref 92가 이긴다. 핵심 3가지만 요약한다:

1. **세아트랜스 메일 전수(30건·첨부 45개) → 14/14 수입건 전부 파싱·검산·배부 성공.** Jino 진단
   *"사실은 fix 될 숫자는 제품원가(CYN) 뿐이고 나머지는 계속 이렇게 업데이트가 되는게 맞거든"*이
   숫자로 확인됐다 — CNY는 14건 동일, 환율 207.51~226.66(엑셀 200 대비 −8.8%), 물류비율
   5.0%~16.3%(3배 차). 관세율은 품목 수준에서 **5.6%가 엑셀이 맞음**을 재확인(로트별 흔들림은
   무관세 부자재 비중 차이). 엑셀 표준원가는 실제 대비 평균 **+7.3% 과소**.
2. **파서 양식변종 4종 수리**(열 밀림·헤더 라벨 어긋남·시트명 `CI (2)`·CI/PL 별도 파일) + CI·PL
   자기검산 추가. Nora(AI 회계)는 재고 평가방법·부가세 질문에 답을 못 했다(엔드포인트가 「내부
   기록 검색」으로 라우팅됨).
3. ★★**AI_office에 이미 수입원가 트랙이 있다**(`track_import_cost.md`·`import_payable_ledger`·
   `compute_landed_cost`·`hs_reference`·ECOUNT 기표 harness 2종). 이 세션이 만든 SellC 수입건
   원장과 **데이터 흐름 0건**, AI_office 쪽은 HTTP 미노출. **어느 쪽이 생산자가 될지 Jino 결정
   필요.**

### 다음 세션이 먼저 볼 것 (우선순위 순)

- ★★**AI_office 수입원가 트랙 중복 처리** — Jino 결정 필요. 데이터 흐름 0건인 두 구현이 병행
  중이니, 결정 전엔 어느 쪽도 상대를 전제하지 말 것(ref 92 §6).
- ★**재고 평가방법** — Nora 미응답. 대안 ⓐSlack @멘션(다른 harness를 탈 수 있음) ⓑ회계기준
  1차 출처 직접 대조(일반기업회계기준 재고자산 장 + 법인세법 시행령 §74)(ref 92 §5).
- **층3-lite 계약 초안** — Fable 제안 순서의 두 번째, (a) 계열 한정 평가 규칙(ref 92 §7).
- 자금정산서 파서 미구현(해상운임이 정산서엔 없어 그것만 쓰면 원가에서 운임이 빠지는 문제 포함) ·
  2025-09~2026-01 서류 공백(다른 포워더/라벨인지 미확인) · 부자재 판정이 품목명 규칙(`clean`·
  `wipe`·`kit`) 의존.

### 이 세션에서 배운 것

- 「계수 검증에는 **구성이 다른 두 건**이 필요하다」 — 한 건으로 판정해 관세율(5.13%)을 오판했다.
  무관세 부자재 비중이 다른 두 번째 건이 있어야 «품목별 세율이 다르다」는 진짜 원인이 드러났다.
- 「**자기검산이 없는 파서는 «값이 있는데 틀린 것»을 만든다**」 — 통관경비서엔 검산이 있었고
  CI·PL엔 없어서 열 밀림 결함이 조용히 통과했다.
