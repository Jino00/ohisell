...
- ~~②NCA 보고서를 한 달 통으로 요청하면 72MB가 오는데 zip이 아니다 — base64 경로가 큰 파일을
  못 견딘다.~~ **오진이었다(같은 날 늦게 정정).** 조각으로 다시 받으니 길이가 정확히 같고
  내용도 멀쩡했다 — 그 바이트는 **탭 구분 TSV**다(188,094행). **쿠팡은 행이 많으면 xlsx 대신
  TSV를 준다.** 다운로드는 처음부터 정상이었고, 진짜 결함은 **포맷이 바뀌는데 파서가 xlsx만
  가정한 것 + 받은 것을 확인하지 않은 것**이었다. TSV 통짜 합계가 주 단위 5개와 차이 0원.
  → 둘 다 수정 완료(아래 §6 (c)(d) 참조). 이제 통짜로 받아도 된다. 단 prod `option-ingest`는
  xlsx만 파싱하므로 **거기에 밀어 넣을 때만** 쪼갠다.
...
- [x] (c) **조용한 실패 수정 완료** — 공용 모듈 `tools/ohitech_ad_cdp.py` 신설. `gql()`이
      fetch throw · 비200 · 로그인 HTML · GraphQL errors를 **전부 예외**로 올리고, `connect()`가
      리로드로 재무장한 뒤 로그인 페이지면 `SessionDead`. 종료코드를 갈랐다 —
      **3=세션 죽음(재시도 무의미) · 4=진짜 0건 · 1=그 외**. 백필 드라이버가 그걸 구분해 보고한다.
      라이브 확인: 세션 없음 → rc=3, 빈 목록 → rc=4, 둘 다 파일을 만들지 않는다.
- [x] (d) **포맷 수정 완료** — 원인은 다운로드가 아니라 **TSV 전환**이었다(위 §4 참조).
      `sniff_format()`이 판정하고 `read_report()`가 xlsx/TSV를 모두 읽는다. 받은 것이 무엇인지
      확인한 뒤에만 저장하고 확장자를 맞춘다. 실데이터 교차 확인: TSV 통짜와 주 단위 xlsx 5개가
      같은 값(2025-10 Retail 3,658,680 · 전환 6,764,090).
      회귀 `tools/test_ohitech_ad_cdp.py` 12건.
