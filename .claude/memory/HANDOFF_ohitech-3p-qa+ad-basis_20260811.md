...
> main에는 없다(§5-1d).**
>
> ## ✅ 2026-08-11 23:4x — §5·§6 전수 실측 완료. **진짜 남은 일 0건.**
> Jino 질문(*"어떻게 뭔가 할때마다 마무리가 안되고 맨날 할 일이 또 생겨?"*)에 답하려고 §6의
> 8건을 하나씩 라이브로 쟀다. **4건이 유령이었고, 나머지는 별건이었다.** 아래 §5·§6의 체크박스는
> **읽지 말고 이 표를 보라**(원문은 이력으로 남긴다). 근거는 교훈 #256.
>
> | 항목 | 실측 결과 |
> |---|---|
> | ★최우선 브랜치 병합(§5-1d) | ✅ **이미 병합됨** — PR #283 `62c456b`. `git log origin/main..claude/ad-basis-truth` 비어 있음 |
> | EZ툴 카메라렌즈 원가(§5-1a) | ✅ **이미 매핑됨** — `product_channel_mapping.id=2906` → `OHI-0225`(2,694원). 2026-08-11 Jino 결정과 동일 |
> | 「28개 옵션 일괄 연결」 | ❌ **실체 없음** — 28개는 14기종 × 1매입/2매입이라 원가가 전부 다르다(일괄 불가). 전 기간 **매출 있는 옵션은 1개**(`87416780317`, 1건 9,800원)이고 그게 이미 매핑된 그것. 나머지 27개 매출 0 → 연결해도 손익 불변 |
> | prod `.deploy-stamp` 부재(§5-1d 아래) | ❌ **오독** — 그 경로는 리뷰 P1-4로 **일부러 폐기**(dist 안이면 `rsync --delete`가 지워 가드 영구 무장해제). 정본 `/home/ubuntu/ohisell/.frontend-deploy-stamp` = `commit=884261b`(main HEAD) **정상 작동** |
> | nginx IP 예외(§5-1b) | Jino 출장 중 = **할 것 없음**. Mac IP가 하루 4번 회전하므로 지금 넣어도 내일 막힌다. 복귀 후 IP 고정되면 기존 허용목록으로 자연 해소, 정체분은 catch-up |
> | 이월 4건(§5-1c) | **별건** — 적대 리뷰가 「이번 범위 밖」으로 치운 P2. 미완이 아니다. 트랙 백로그로 |
> | 디스크 | 87%(여유 14GB) — 08-10 83%에서 재상승. 관측만, 당장 위험 아님 |
>
> **D-CPP-38 계약은 완료다** — prod 라이브 합격 + 적대 리뷰 P1=0 + main 병합. 다시 손댈 것 없다.
