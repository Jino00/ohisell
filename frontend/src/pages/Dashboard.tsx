    ...
    <div className="max-w-7xl mx-auto">
      ...
      {bulkPanelOpen && (
        <div className="mb-6 rounded-lg border border-gray-200 bg-white px-4 py-3">
          ...
          {/* ★고정 처방을 쓰지 않는다(2026-08-23 적대 리뷰 P1-2). 종전 문구는 「로그인 필요
              표시가 뜨면 … 로그인한 뒤 누르세요」였는데, 자동 재개가 배선된 레인의 줄에는
              바로 위에 「누를 필요 없이 자동으로 이어받습니다」가 떠서 **한 화면에서 두 처방이
              모순**됐다. 처방은 레인마다 다르므로(자동 재개 배선 유무) 저자는 outcomeView
              하나여야 하고, 여기서는 「줄마다 다르다」는 사실만 말한다. */}
          {!bulkRunning && (
            <div className="mt-2 text-xs text-gray-500">
              처방은 줄마다 다릅니다 — 각 항목의 안내를 그대로 따르세요. 「자동으로 이어받습니다」로
              적힌 레인은 Mac에서 로그인만 하면 되고, 그 밖의 실패는 <b>전체 갱신</b>을 다시 누르면
              재시도됩니다.
            </div>
          )}
        </div>
