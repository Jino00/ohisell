...
//
// ⚠️**짝 파일: `periodRangeBarClick.test.tsx`.** 여기는 순수 함수만 본다 — 버튼의
//   **클릭 동작**이 이 함수를 안 쓰게 바뀌면 못 잡는다(2026-08-14에 실제로 그랬다).
//   짝 파일이 클릭 경로를 덮어 고리를 닫는다. 둘은 같이 살고 같이 죽는다(적대 리뷰 3R 이월).
