...
| `oscillation_daycount.py` | D-NAO-288 §4-C ⓗ — **「같은 소재·같은 날 UP∧DOWN이 함께 난 날」 수**를 배포 전/후 두 창으로. ★배포일은 양쪽 창에서 제외하되 그 날의 수와 배포 전/후 분해를 병기(D-NAO-292) · ★진행 중인 날은 분모에서 제외 · ★분모 둘(완결된 날 / 발화가 있었던 날) | ref 138 · 같은 계약 §4-C ⓗ |
...
★★**셋 다 창·소재·컷오프를 «출력에» 찍는다**(2026-09-06, D-NAO-292). 안 찍었을 때 무슨 일이 났는지가
이 규율의 근거다 — 계약 §0의 「27/54」는 **분자가 소재 1개, 분모가 전 소재**인 비율이었고,
세 세션이 지나도록 아무도 못 봤다(정본 `ref 138` §1). **비율을 적을 땐 창·grain·컷오프를 떼지 않는다.**
