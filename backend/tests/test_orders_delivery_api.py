...
#       ③실부담 합계가 조회되는가(N배송 3,377−3,000=377 / 일반 1,900−0=1,900)
...
def test_order_list_adds_delivery_fields_without_breaking_shape(client):
    ...
    assert Decimal(nb[0]["shipping_cost_paid"]) == Decimal("3377")
    assert Decimal(nb[0]["shipping_cost"]) == Decimal("3000")
    assert Decimal(nb[0]["shipping_cost_net"]) == Decimal("377")  # 실부담 = 3,377 − 3,000
...
def test_delivery_breakdown_reproduces_four_combinations(client):
    ...
    # 실부담: N배송 3건 × (3,377−3,000)=1,131, 일반 TODAY 6건 × 1900=11,400
    assert Decimal(got[("ARRIVAL_GUARANTEE", True)]["net_total"]) == Decimal("1131")
    ...
    assert Decimal(d["total_paid"]) == Decimal("3") * Decimal("3377") + Decimal("9") * Decimal("1900")
