...
def _claimed_lease(seed, account_key: str) -> str:
    ...
    """
    rc.request_refresh(seed, account_key)
    lease = rc.claim_refresh(seed, account_key)["lease"]
...
def test_reports_error_with_valid_token(env, url, account):
    client, seed = env
    lease = _claimed_lease(seed, account)
...
def test_missing_error_field_records_unknown(env, url, account):
    ...
    client, seed = env
    r = client.post(url, json={"lease": _claimed_lease(seed, account)},
                    headers={"X-Ingest-Token": _TOKEN})
    assert r.status_code == 200
