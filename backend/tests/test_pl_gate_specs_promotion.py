...
def test_SPECS_인구조사_현재_11종(db):
    """키가 늘면 이 줄이 빨개진다 — 「무엇이 학습 가능한가」가 조용히 안 바뀌도록.

    ★2026-08-31(D-NAO-282·계약 P2-ⓑ): 7 -> 9. 는 둘은 봉투가 아니라 **킬스위치**다
    (`ad_bid_routing_enabled`·`naver_cs_dry_run`). 종전엔 배포·재시작으로만 바뀌었고
    «지금 켜져 있는지»가 화면 어디에도 없었다.

    ★2026-09-04(D-NAO-286·계약 「표본하한 목표」): 9 -> 11. 는 둘은 **표본 하한**이다
    (`min_weekly_conv_campaign`·`min_weekly_conv_target`). 값 15는 `ref 33` [6] 원문이고
    `ref 65` §4 판정표가 이미 게이트 상수로 지목한 수다 — 새로 만든 숫자가 아니다.
    """
    assert set(guardrail_params.SPECS) == {
        ...
        "min_weekly_conv_campaign", "min_weekly_conv_target",
    }
