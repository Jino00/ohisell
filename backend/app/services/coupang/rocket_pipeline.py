...
def compute_rocket_ri_queue(db: Session, vendor_id: str | None) -> dict:
    ...
    return {
        ...
        "last_collection_date_kst": last_day,
        # ★사용자에게 그대로 보이는 문구다. 「확인」을 쓰지 않는다 — 이 화면에서 「확인」은
        #   「거래명세서확인」(사람이 눌러 RI→CI로 보내는 동작)이라 조회 신선도에 같은 낱말을
        #   쓰면 «아직 안 누른 건»으로 읽힌다(2026-08-28 Jino 지시).
        "note": (
            "「지금 상태를 모르는 건」은 최근 수집에서 다시 보지 못한 발주입니다. "
            "화면의 상태는 «지금»이 아니라 마지막으로 봤을 때의 것이라, 그 사이 이미 "
            "처리됐을 수 있습니다 — 「미종결 발주 재수집」을 돌리면 지금 상태로 갱신됩니다."
        ),
    }
