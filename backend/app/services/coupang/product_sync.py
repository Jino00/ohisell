...
def _maybe_upsert_mapping(db: Session, channel: Channel, detail: dict, item: dict) -> bool:
    ...
    if mapping is None:
        ...
        db.add(mapping)
    elif mapping.mapping_source in ("excel_master", "manual"):
        # 상품 연관맵 트랙 D-6/D-12: 엑셀 마스터(진실원천) 또는 화면에서 수동 편집(manual)한
        # 매핑은 SKU 매칭 자동동기화가 덮어쓰지 않는다(스케줄러 주기마다 조용히 재배정되는 회귀 방지).
        return False
