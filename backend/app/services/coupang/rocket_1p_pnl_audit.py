...
#   주입한 응답(`screen`)을 받는다. 가드가 막는 것은 **화면 모듈 참조**이고, 그래서 화면이
#   낸 숫자를 **다시 계산할 길이 없다** — 사다리·폴드·광고 귀속은 전부 주입된 응답 안에서만
#   읽는다. (DB는 여전히 받는다: A6·B2의 원료는 «돈 축 정본» 모듈에서 온다. 즉 「어떤 계산도
#   못 한다」가 아니라 「화면의 숫자를 재도출할 수 없다」가 정확한 표현이다.)
#
# ★판정은 셋이다: pass / fail / undetermined. 판정할 수 없는 검사를 pass로 칠하면 그게
#   거짓 초록이다(교훈 #123). **이 파일에서 가장 자주 틀리는 자리가 «수집이 안 된 것»을
#   «없는 것»으로 읽는 것이다** — A6·B2가 그래서 신선도·모집단을 먼저 본다.
...
def _promo_sync_state(db: Session, vendor: str, date_to: date) -> tuple[str | None, bool]:
    ...
    ★한계(정직하게): 이건 «수집기가 마지막으로 성공한 시각»이 아니라 «행이 마지막으로
      갱신된 시각»이다. 수집은 돌았는데 그 vendor의 프로모션이 0건이면 갱신할 행이 없어
      시각이 전진하지 않는다 → 그 경우 이 함수는 «안 덮는다»고 답하고 A6은 undetermined가
      된다. 거짓 초록보다 거짓 «모름»이 낫다는 선택이다.
    """
    raw = db.execute(
...
def compute_pnl_audit_checks(db: Session, date_from: date, date_to: date,
    ...
    ★`screen`은 1P 매출·손익 화면 SA의 응답이며 **라우터가 주입한다**(`ATOM_LIMIT`으로
      ...
      ② 그 결과 **화면이 낸 숫자를 재도출할 길이 없다** — 「근거 창은 계산을 새로 하지
         않는다」가 문서 규칙이 아니라 **구조**가 된다.
    ...
    if pc is None:
        ...
        if pc["promos"] == 0:
            # ★★여기가 거짓 초록이 나던 자리. 「행 0건」은 «프로모션이 없었다»일 수도,
            #   «수집이 안 됐다»일 수도 있다 — 신선도가 창을 덮을 때만 전자로 단정한다.
            if fresh:
                ...
                checks.append(_check("A6", "창에 걸친 프로모션 전건에 할인액 원천", 0, 0,
                                     ...
                                     note=f"{seen} — 창 끝({date_to.isoformat()})을 덮지 못해 "
                                          "«프로모션이 없었다»와 «수집이 안 됐다»를 가를 수 "
                                          "없습니다."))
        else:
            ...
            checks.append(_check("A6", "창에 걸친 프로모션 전건에 할인액 원천",
                                 ...
                                 note=(f"{seen}." if pc["unpriced"] == 0 else
