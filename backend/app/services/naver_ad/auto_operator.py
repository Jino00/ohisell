...
# ══════════ D-NAO-288 S1: 시간당 레인 네 게이트가 «어떤 창을 보는지» 한 자리 선언 ══════════
# ★왜 표로 두나 (2026-09-05 실측): 진동의 원인은 게이트마다 창이 다른 것 자체가 아니라, **그
#   사실이 코드 네 군데에 흩어져 아무도 나란히 못 보던 것**이었다. 소재 nad-…554755092는
#   4일 동안 54회 발화·16회 실쓰기로 1,320원 → 1,330원(순변위 +0.8%)을 돌았다 —
#   09-03은 CPC급등이 14시간 연속, 09-04는 순위고삐가 12시간 연속 래치로 걸렸다.
#
# ★두 열이 핵심이다:
#   · `window`     = 그 게이트가 실적을 읽는 창
#   · `self_mixed` = 그 창에 **우리 자신의 당일 쓰기 결과**가 섞이는가.
#     True인 게이트는 «자기가 만든 결과를 자기 판정 근거로 쓴다» — 되먹임의 좌표다.
#
# ★이 표는 «선언»이고, 선언만 있는 표는 다음 세션에 조용히 거짓이 된다(ref 131 §5 신3).
#   그래서 `test_naver_oscillation_damping.py`가 각 게이트의 실제 창 소비를 대조한다.
_WINDOW_SETTLEMENT = "settlement_d8_d2"   # 정착창 [오늘-8, 오늘-2] — _settlement_window()
_WINDOW_TODAY_CURVE = "today_hh24"        # 당일 hh24 곡선 누적 — fetch_entity_hh24()
...
_HOURLY_DECISION_WINDOWS: dict[str, dict] = {
    ...
    "cpc_spike": {
        ...
        "self_mixed": True,
        "note": "당일 CPC > 정착창 CPC×비율. **분자가 우리 상향으로 오른다** — D-NAO-288 B-veto 대상.",
    },
    "loss_leash": {
        "side": "brake", "baseline_window": _WINDOW_SETTLEMENT, "reads_today_curve": True,
        "self_mixed": True,
...
# ★★D-NAO-288 미러 거부권 2개 — 둘은 **한 커밋으로만** 들어간다(계약 §2-2).
#   하나만 넣으면 §7 「액셀·브레이크 대칭」이 깨진다: B만이면 안전장치 단독 완화이고,
#   A만이면 D-NAO-85(ROAS +7% · 매출 −52%)의 재현이다.
#   둘 다 `hold`만 만든다 — 새 액션(상향/하향)을 **만들지 않는다**. 그래서 대칭이다.
#
#   A-veto(액셀): 정착창 ok여도 **오늘 추정ROAS가 BEP 아래로 확인되면** UP 금지.
#     지금 그 증거는 이미 손에 있다 — 고삐가 매시 계산한다. 다만 고삐는 «당일소진 ≥ 하루평균»을
#     기다리므로 그 대기 구간(09-04 11:20 실측)에는 브레이크가 침묵하고 액셀만 발화했다.
#     ★고삐의 소진 문턱은 건드리지 않는다 — 하향은 여전히 하루치 소진 뒤에만 나간다.
#   B-veto(브레이크): 당일 CPC 상승이 **우리 자신의 당일 상향분으로 설명되는 범위 안**이면
#     CPC급등 DOWN을 발동하지 않는다. 오늘 우리 쓰기가 0건이면 배수 1.0 = 종전과 완전 동일.
...
# ── D-NAO-171(B-3): 그룹 판정 → 전 소재 집행 ──
# 되돌림 스위치. False면 max 소재 1개만 스텝하던 D-NAO-171 이전 동작으로 복귀한다
# (`AD_BID_ROUTING_ENABLED=False`는 그보다 위에서 ad 라우팅 전체를 덮는다 — 두 스위치는 층이 다르다).
_GROUP_STEP_ALL_ADS: bool = True

# 레인 1회차당 소재 **쓰기** 상한(계정 전체 합). `_MAX_AD_AUTO_EXEC_PER_LANE`(=5)은 «결정» 수를
# 세고 이 상수는 «쓰기» 수를 센다 — 둘을 나눈 이유: 캡의 존재 이유는 "규칙이 틀렸을 때 되돌릴
# 시간"인데, **틀리는 단위는 판정(그룹)이지 소재가 아니다.** 그래서 그룹 결정 1건은 소재가
# 몇 개든 결정 캡 1건으로 세고, blast radius는 이 별도 쓰기 상한으로 잡는다.
# 15인 이유: 죽은 그룹의 평균 소재 수 실측 ≈1.7(207그룹·351소재), 5결정 × 대형 그룹 혼입
# 여유. 초과분은 버리지 않고 Confirm 대기로 강등한다(조용한 드롭 금지 — D-NAO-125 관례).
...
def _bid_from_change_snapshot(raw: str | None) -> int | None:
    """`naver_change_log.before_value`/`after_value`(엔티티 JSON 원문)에서 입찰가를 꺼낸다.

    grain마다 자리가 다르다(prod 실측 2026-09-05): 소재는 `adAttr.bidAmt`(155행),
    광고그룹·키워드는 최상위 `bidAmt`(95행). 둘 다 못 찾으면 None —
    **모르면 배수 1.0으로 떨어져 브레이크가 종전대로 산다**(fail-closed, `_own_bid_multiple_today`)."""
    if not raw:
        return None
    try:
        obj = json.loads(raw)
...
def _own_bid_multiple_today(
    ...
    """★D-NAO-288 B-veto 원료 — **오늘 우리가 이 유닛의 입찰가를 몇 배로 올려 놨나.**

    (배수, 사유) 반환. 오늘 00시 입찰가(= 오늘 첫 실쓰기의 `before_value`) 대비 현재
    입찰가(= 오늘 마지막 실쓰기의 `after_value`)의 비.

    왜 필요한가(2026-09-03 실측): CPC급등 DOWN은 «당일 CPC > 정착창 CPC × 2»로 판정하는데,
    분자인 당일 CPC는 **우리가 입찰가를 올리면 따라 오른다**. 09-03에 이 유닛은 우리 손으로
    1,320 → 1,980원(×1.5)이 됐고, 그 결과로 오른 CPC 1,683원이 곧바로 우리 브레이크의
    근거가 됐다 — 14시간 연속 DOWN. **엔진이 만든 결과가 엔진의 정지 근거였다.**

    ★배수는 **1.0 아래로 내려가지 않는다.** 오늘 우리가 «내렸으면» 배수가 1 미만이 되어
    문턱이 낮아지고 브레이크가 더 쉽게 걸리는데, 그건 감쇠가 아니라 **브레이크 단독 강화**다
    (§7 대칭 위반). 이 거부권은 브레이크 발화를 **줄이기만 하고 늘리지 않는다**.
...
def _intraday_loss_leash(
    ...
    총이익 절대액 극대화 — 볼륨 0=이익 0이므로 kill보다 leash가 우월).
    **(발동여부, 사유, 오늘_BEP하회_확인)** 반환.

    ★D-NAO-288: 세 번째 값 = 「오늘 추정ROAS가 BEP 아래인가」의 **3상** 판정
    (`True`=하회 확인 · `False`=하회 아님 확인 · `None`=판정 불가). 발동여부와 «다른 질문»이다 —
    발동은 여기에 ④소진 문턱을 **더** 요구하기 때문이다. 그 차이가 곧 §7 비대칭의 좌표였다:
    2026-09-04 11:20 실측에서 이 값은 이미 True였는데(추정ROAS 1.47 < BEP 2.00) 당일소진이
    하루평균 6,919원에 못 미쳐 고삐는 침묵했고, 같은 시각 액셀은 정착창 3.7176만 보고 올렸다.
    A-veto(`_judge_hourly`)가 이 값을 소비해 **그 창에서 상향만 멈춘다** — 하향은 안 만든다.
    ★값을 여기서 내는 이유: 판정창과 실행 재검증창이 갈라지지 않게 **한 번 계산한 것을 그대로
    넘긴다**(계약 §2-4 · D-NAO-265 재발 방지).
    ...
    adgroup_id = _resolve_adgroup_id(db, target_type, target_id)
    if adgroup_id is None:
        return False, "adgroup 해석 불가", None
    ...
    if price is None or bep_roas is None:
        return False, "상품 단가/BEP 미확인 — 고삐 판정 불가", None

    est_roas = intraday_roas.estimated_intraday_roas(curve, price)
    if est_roas is None:
        return False, "당일 소진 없음", None

    # ★D-NAO-288: 여기서부터 「오늘 BEP 하회인가」는 **확정**이다 — 아래 ④소진 문턱은 발동을
    # 가르지만 이 사실을 바꾸지 않는다. 두 질문을 분리해 내보낸다.
    sub_bep = bool(est_roas < bep_roas)

    today_cost = sum(h["cost"] for h in curve)
    avg_daily_cost = baseline_agg["cost"] / _HOURLY_BASELINE_DAYS
    if avg_daily_cost <= 0:
        return False, "정착창 소진 기준 없음", sub_bep
    if today_cost < avg_daily_cost:
        return False, "당일 소진<하루평균 — 판정 유보(과소추정 방어)", sub_bep
    ...
    return False, f"장중 추정ROAS {est_roas} ≥ BEP {bep_roas} — 고삐 불발", sub_bep
...
def _judge_hourly(
    ...
    우선순위(하나만, DOWN이 UP보다 먼저 = bleeding day UP 금지, §0 불변 가드):
      ...
      1. CPC 급등 DOWN(trigger_watch.CPC_SPIKE_RATIO). ★D-NAO-288 **B-veto**: 문턱에서
         «우리 자신의 당일 상향 배수»를 벗긴다 — 자기유발 CPC 상승은 급등이 아니다.
      2. 장중 loss 고삐 DOWN(D-NAO-60 RL3 — 추정 ROAS<BEP, 안전방향 한정 완화). 상단 순위의
         무전환 고지출도 여기서 잡힌다(전환 0 → est ROAS 0 < BEP → 고삐, IU2 안전망).
      2.5 ★D-NAO-288 **A-veto**(GATE P2-A-3): 고삐가 «오늘 BEP 하회»를 확인했는데 소진 문턱
         미달로 발동을 미룬 구간 — 그 구간에서 **UP만 멈춘다**(hold). 하향은 안 만든다.
    ...
    ★★D-NAO-288 — 그 「비대칭 기억」이 되먹임 진동을 낳았다(prod 4일 실측, 계약 §0):
    액셀은 과거창만 보고 브레이크는 당일 누적만 보는데, **당일 누적은 우리 상향의 결과를 담는다.**
    소재 nad-…554755092는 54회 발화·16회 실쓰기로 1,320 → 1,330원(+0.8%)을 돌았다.
    두 거부권은 그 «비대칭 기억»을 지우지 않는다 — 창은 그대로 두고, **자기 창이 상대에게
    오염됐을 때 자기 발화만 멈춘다.** 둘 다 hold만 만들어 §7 대칭을 깨지 않는다.
    창의 선언은 모듈 상단 `_HOURLY_DECISION_WINDOWS` 한 자리."""
    ...
    cpc_spike_self_caused = ""  # D-NAO-288 B-veto가 CPC급등 DOWN을 눌렀을 때의 사유문
    ...
    # ★D-NAO-288 S1: 창 경계는 «선언표»에서 받는다(`_gate_baseline_window`) — 표가 집행되는 자리.
    ...
    if baseline_cpc is not None and baseline_cpc > 0 and today_cpc is not None:
        # ★★D-NAO-288 B-veto — 분자(당일 CPC)에 섞인 «우리 자신의 당일 상향분»을 문턱에서 벗긴다.
        ...
        if today_cpc > spike_threshold:
            return {
                ...
                "reason": (
                    ...
                    f"×{CPC_SPIKE_RATIO}"
                    + (f"×자기상향 {float(own_multiple):.2f}배" if own_multiple > 1 else "")
                ),
                **base,
            }
        if own_multiple > 1 and today_cpc > baseline_cpc * CPC_SPIKE_RATIO:
            ...
            log.info(
                "auto_operator: CPC급등 보류(자기유발분, D-NAO-288) target=%s 당일=%.1f원 ≤ "
    ...
    leash_fired, leash_reason, today_sub_bep = _intraday_loss_leash(
    ...
    # ★★GATE P2-A-3 = **D-NAO-288 A-veto** — 위 P2-A-1의 거울. P2-A-1이 「정착창이 나쁘다고
    ...
    if settle_status == "ok" and today_sub_bep is True and today_conv >= _INTRADAY_UP_MIN_CONV:
        return {
            "direction": "hold",
            "reason": (
                f"UP 보류(오늘 증거 거부권, D-NAO-288 A-veto) — 정착창({settle_reason})인데 "
...
def _exploration_daily_loss_reason(db: Session, adgroup_id: str, today: date,
                                   now: datetime | None = None,
                                   effective_bid_value: int | None = None) -> str | None:
    ...
    비용)를 읽어 `effective_bid_value × account_diagnosis.LOW_CLICK_THRESHOLD`(스톱로스 보드 자신이
    이미 쓰는 «표본 부족» 정의, `account_diagnosis.py:819`)와 비교한다. 미만이면 손실 확정이 아니라
    판정 불가이므로 통과(None) — 단 두 번째 조건(스톱로스 보드)은 그 뒤에도 그대로 검사한다."""
    bep_reason = _bleeding_hold_reason(db, "adgroup", adgroup_id, today, now)
    if bep_reason is not None:
        exempted = False
        if bep_reason == "④최신 소급채점에서 bleeding으로 판정됨" and effective_bid_value is not None:
            ...
            if cost_asof is not None:
                ...
                if cost_asof < stop_loss_amount:
                    ...
                    return (
                        f"④bleeding ∧ 14일 비용 {cost_asof}원 ≥ 스톱로스 "
                        f"{stop_loss_amount}원(D-NAO-289)"
                    )
        if not exempted:
...
def _run_exploration_for_campaign(
    ...
    for _etype, adgroup_id in candidates:
        ...
        # D-NAO-289: 가드5의 «표본 부족» 예외 판정에 실효입찰이 필요하므로, 레버 맞춤 라우팅이
        # 쓰는 그 파생(effective_bid)을 손실검사보다 먼저 계산해 한 번만 쓴다(값 재계산 금지).
        ...
        try:
            ...
            eff = None
        eff_bid_value = eff["effective_bid"] if eff is not None else None

        # GATE P2-risk6(가드5 완성): 활성 daily 손실 상태(스톱로스/floored/바닥손실) 그룹 제외 —
        # 어제 daily 고삐가 조인 그룹을 오늘 탐색 UP으로 역전하지 않는다(DL과 동일 보드·신선도).
        # D-NAO-289: eff_bid_value가 있으면 가드5 내부에서 «표본 부족» 예외를 검사한다(없으면
        # 종전대로 fail-closed — 실효입찰 파생 실패를 「모름=괜찮음」으로 읽지 않는다).
        loss_reason = _exploration_daily_loss_reason(
            db, adgroup_id, today, now, effective_bid_value=eff_bid_value)
...
def run_hourly_lane(db: Session, *, now: datetime | None = None, fetch_intraday=None) -> dict:
    ...
    result: dict = {
        ...
        # D-NAO-171(B-3): 그룹 결정 1건이 소재 N건으로 팬아웃된 결과의 관측 원료.
        # ad_fanout_groups = 소재 2개 이상으로 펼쳐진 «그룹 결정» 수(N=1은 종전과 같으므로 안 센다).
        # ad_writes_reserved = 이번 레인이 예약한 소재 쓰기 총수(_MAX_AD_WRITES_PER_LANE 대상).
        "ad_fanout_groups": 0,
        "ad_writes_reserved": 0,
        "ad_writes_capped": 0,  # 쓰기 상한 초과로 Confirm 대기로 강등된 소재 수
    ...
    for campaign_id in campaigns:
        ...
        for target_type, target_id in _hot_set_candidates(db, campaign_id, window_from, window_to):
            ...
            # ★★D-NAO-288: 미러 거부권 2건은 **운영 일기(blocked)에 남긴다.** 아래 판정 hold가
            # 일기에 안 남는 이유는 "액션 의도 자체가 없었으므로"(`_record_blocked` 선별 규약)인데,
            # 거부권은 정반대다 — **올리려던/내리려던 것을 멈춘 것**이라 정확히 그 규약이 말하는
            # 「의도된 액션이 차단된 것」이다. 로그로만 두면 안 된다: 블루-그린 배포가 pm2 로그
            # 파일을 옮겨 배포 «전» 발화가 활성 로그에서 조용히 사라진다(2026-08-28 실측).
            ...
            # D-NAO-171: ad 라우팅에서만 채워지는 집행 대상 목록(비-ad는 빈 리스트 = 단건 경로).
            ad_exec_targets: list[tuple[str, int]] = []
            if target_type == "adgroup":
                try:
                ...
                if eff is not None and eff["source"] == "ad":
                    ...
                    if hold_reason is not None:
                        ...
                        continue
                    # ★D-NAO-171(B-3): 그룹 판정의 **집행 대상은 그 그룹의 활성 false 소재 전부**다.
                    #   종전엔 max 소재 1개만 스텝했는데, 그러면 판정 grain(그룹 집계)과 집행
                    #   grain(소재 1개)이 어긋난다. DOWN에서 max만 −15%하면 **차순위 소재가 새 max**가
                    #   되어 그룹 실효는 소재 간 갭만큼만 내려간다(손실 국면에서 가장 느린 실패).
                    #   UP도 대칭으로 반쪽이 된다 — D-NAO-59(총이익 최대화)가 ROAS 방어로 표류하는
                    #   그 방향이다. 실측(2026-08-10): 그룹입찰 死 207그룹 중 **156개(75%)는 소재
                    #   1개**라 이 변경의 영향이 없고(N=1은 종전과 완전 동일), 문제는 **소재 2~10개
                    #   이면서 입찰가가 제각각인 46그룹**이다.
                    #   ★승수 스텝(각자 기준 ±15%)이라 **소재 간 비율이 보존**된다 — 상품별 차등은
                    #     우리가 만든 구조가 아니므로 평탄화하지 않는다(이유를 모르는 구조는 보존).
                    #   ★봉투 계수는 하나도 안 바꾼다: 쿨다운·자동하향 상한·누적 상향 상한은 전부
                    #     소재(entity) 단위인데, 전 소재가 한 회차에 락스텝으로 같이 스탬프되므로
                    #     그룹 실효 기준 케이던스가 **자동으로** 단일소재 그룹과 같아진다.
                    #
                    #   ★라이브 목록 1콜로 받는다(get_ad_bid를 소재마다 N번 부르지 않는다):
                    #     ①API 콜이 N→1로 준다 ②DB(`naver_adgroup_product`)는
                    #     (adgroup_id, mall_product_id) 유니크라 **같은 상품의 소재가 여럿이면
                    #     첫 하나만 남는다**(shopping_ad_product_sync의 dedup) — 집행 대상 목록을
                    #     DB에서 뽑으면 조용히 빠지는 소재가 생긴다. 라우팅 판정(source='ad')은
                    #     종전대로 DB 파생을 쓰고, **집행 대상과 스텝 기준만 라이브**로 잡는다.
                    try:
                        live_ads = get_ads(target_id)
                    except Exception as e:  # noqa: BLE001 — 목록 조회 실패는 hold(fail-closed)
                        log.warning("auto_operator: 소재 목록 라이브 조회 실패 adgroup=%s: %s", target_id, e)
                        live_ads = None
                    # 집행 대상 = 활성(userLock=False) ∧ useGroupBidAmt가 **명시적 False** ∧ 입찰가 존재.
                    # None(파싱 실패·부재)은 제외한다 — 그룹입찰이 실효일 수 있는 소재에 개별 bidAmt를
                    # 쓰면 writer가 어차피 fail-closed로 거부한다(§0 강제 전환 금지의 대칭).
                    for a in live_ads or []:
                        if a.get("ad_user_lock") is True or a.get("use_group_bid_amt") is not False:
                            continue
                        a_id, a_bid = a.get("ad_id"), a.get("ad_bid_amt")
                        if a_id and isinstance(a_bid, int):
                            ad_exec_targets.append((a_id, a_bid))
                    if not _GROUP_STEP_ALL_ADS:
                    ...
                    if not ad_exec_targets:
                        ...
                        continue
                    # 스텝 기준의 대표값 = max 소재(종전과 동일) — 아래 rank-step 분기는 ad에
                    # 걸리지 않으므로(exec_target_type이 adgroup/keyword가 아님) 실제로는
                    # _clamp_step 폴백만 타고, 소재별 스텝은 집행 루프에서 각자 기준으로 다시 계산한다.
                    ad_exec_targets.sort(key=lambda t: t[1], reverse=True)
                    exec_target_type = "ad"
                    exec_target_id = ad_exec_targets[0][0]
                    exec_adgroup_id = target_id
                    step_base = ad_exec_targets[0][1]
            ...
            if is_probe:
                ...
                proposal_approval_source = APPROVAL_SOURCE_HOURLY
            # ★D-NAO-171(B-3) 집행 팬아웃 — 그룹 결정 1건 → 소재 N건.
            # 비-ad 경로는 `ad_exec_targets`가 비어 있어 종전과 **완전히 같은 단건 1회전**을 돈다
            # (아래 폴백 `[(exec_target_id, step_base)]`가 종전 값 그대로다). ad 경로만 N회전한다.
            # ★각 소재의 스텝은 **자기 현재 입찰가 기준**으로 다시 계산한다(승수 스텝) — 그래야
            #   소재 간 비율이 보존되고, 각 소재의 변경폭이 자기 기준 ±15%라 per-ad 가드레일이
            #   구성상 항상 정합한다. 그룹 실효(=max)도 정확히 ±15% 이동한다(max 소재가 자기
            #   15%를 스텝하므로).
            # ★부분 실패는 허용한다: 소재 j가 쿨다운·CAS·가드에 막히면 그 소재만 skip된다.
            #   그룹 내 비율이 한 회차 최대 15% 틀어지지만 다음 회차가 각자 현재값 기준으로
            #   다시 밟으므로 틀어짐은 유계이고 재수렴한다. 네이버 API에 배치 쓰기가 없어
            #   원자성은 어차피 허구다 — 있는 척하지 않는다.
            if len(ad_exec_targets) > 1:
                result["ad_fanout_groups"] += 1
            # ★결정 캡과 쓰기 캡은 «층이 다르다»(D-NAO-171): 이 그룹 결정은 소재가 몇 개든
            #   `_MAX_AD_AUTO_EXEC_PER_LANE`(결정 5건)에서 **한 자리만** 쓴다 — 캡의 존재 이유가
            #   "규칙이 틀렸을 때 되돌릴 시간"이고 틀리는 단위는 판정이지 소재가 아니기 때문이다.
            #   대신 blast radius는 `_MAX_AD_WRITES_PER_LANE`(쓰기 15건)이 잡는다.
            ...
            for _fan_idx, (exec_target_id, exec_step_base) in enumerate(
                ...
                if exec_target_type == "ad":
                    ...
                    if step_bid is None:
                        ...
                        _record_blocked(
                            db, campaign_id=campaign_id, actor=lane_actor, reason=hold_reason,
                            now=now, target_type="ad", target_id=exec_target_id,
                            action=intended_action,
                        )
                        continue
                ...
                # ★D-NAO-171: 캡이 둘이다. ①결정 캡 — 이 그룹이 아직 자리를 안 잡았을 때만 본다
                ...
                if lane_capped:
                    ...
                    if write_capped:
                        result["ad_writes_capped"] += 1
                    log.warning(
                        "auto_operator: 소재 자동 실행 캡 도달(결정 %s/%d · 쓰기 %s/%d) — "
                        "proposal_id=%s ad=%s는 Confirm 대기로 강등(D-NAO-125·171)",
                        result["ad_auto_exec_reserved"], _MAX_AD_AUTO_EXEC_PER_LANE,
                ...
                if exec_target_type == "ad":
                    ...
                    if not group_slot_taken:
                        result["ad_auto_exec_reserved"] += 1
                        group_slot_taken = True
                    result["ad_writes_reserved"] += 1  # 쓰기는 소재마다 센다(blast radius)
                if is_probe:
                    result["probed"] += 1
