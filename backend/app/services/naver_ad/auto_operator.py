...
# ★D-NAO-129 최종(2026-07-29, codex 적대 4R + Claude 독립 판단 합치): **상향은 생성만 열고
#   자동 발사는 켜지 않는다.** bid_up은 위 생성 게이트에는 있고 여기에는 없다 = Confirm 큐로 간다.
#
#   왜 — 상향의 경제적 브레이크가 **대리 지표**이기 때문이다. BEP는 소재가 아니라 부모 그룹의
#   30일 집계라, 같은 그룹에 매출 내는 소재가 하나만 있어도 전환 0인 소재가 그 실적을 빌려
#   계속 통과한다. 그래서 가격 규칙(±15%·기준가 2배)으로 천장을 합성했는데, 그 기준점을
#   지키는 장치가 **구조적으로 성립하지 않는다**: 외부(대행사) 하향을 기준점 재설정 사건으로
#   쓰려면 D-NAO-127 탐지가 그 사건을 봐야 하는데, 탐지는 하루 1회(07:45)이고 시간당 레인은
#   매시간 쓴다 — 우리가 먼저 쓰면 editTm이 우리 것으로 바뀌어 **외부 사건이 영구 소실**된다
#   (codex 적대 4R P1, 재현: 07:45 기준 2,000 → 10:00 대행사 400 → 11:00 우리 460 → 다음날
#   탐지는 "우리 쓰기"로 분류 → 기준점은 영영 2,000 → 자동화가 4,000까지 되돌린다).
#   하향은 이 문제가 없다 — 판정이 틀려도 지출이 주는 방향이다. 상향은 반대다.
#
#   ★자동 발사를 켜기 전 선행조건(둘 중 하나):
#     ① 소재별 경제 신호(소재 단위 전환·매출 귀속)를 만들어 BEP를 소재 grain에서 판정한다
#        — 그러면 가격 규칙이라는 대리 지표 자체가 필요 없다(codex·Claude 공통 권고).
#     ② 외부 변경을 **쓰기 직전에** 확인하는 경로를 만든다(레인이 이미 라이브 재조회를 하므로
#        그 값과 마지막 관측을 대조하면 된다) — 그래야 기준점이 실시간으로 재설정된다.
#   그때까지 상향은 사람이 카드를 보고 승인한다. 켜는 방법은 이 집합에 "bid_up" 한 줄.
