...
def add_restricted_keywords(adgroup_id: str, keywords: list[str]) -> WriteResult:
    ...
    Raises:
        WriteValidationError: keywords 빈 값 / adgroup이 WEB_SITE 아님 / 이미 등록된 키워드 재등록.
        WriteError: POST가 2xx 아님(재시도 없음 — 비멱등 쓰기).
        WriteVerificationError: POST는 2xx였는데 재조회에 반영 안 됨(fail-closed).
    """
    if not keywords:
    ...
    if isinstance(response_body, list):
        created_ids = [
            row.get("nccAdgroupRestrictKwdId")
            for row in response_body
            if row.get("nccAdgroupRestrictKwdId")
        ]
    if not created_ids:
        ...
        created_ids = [
            row.get("nccAdgroupRestrictKwdId")
            for row in after
            if row.get("keyword") in keyword_set and row.get("nccAdgroupRestrictKwdId")
        ]
...
def delete_restricted_keywords(adgroup_id: str, restrict_kwd_ids: list[str]) -> WriteResult:
    ...
    Raises:
        WriteValidationError: restrict_kwd_ids 빈 값.
