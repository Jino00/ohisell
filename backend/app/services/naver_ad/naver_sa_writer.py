...
def get_shopping_exclusions(adgroup_id: str) -> list[dict]:
    ...
    for target in resp.json():
        ...
        if items is None:
            # ★`None`은 **「제외를 한 번도 설정한 적 없다」**이지 스키마 이상이 아니다
            #   (2026-08-17 라이브 대조군으로 확정):
            #     · `None`인 3그룹(`01. Z플립8`·`02. Z폴드8 와이드`·`03. Z폴드8 울트라`, 전부
            #       2026-07-27 생성·PAUSED)은 `regTm == editTm` — **한 번도 편집된 적이 없다.**
            #     · 반면 `[]`인 `S26`은 우리가 PUT으로 비운 그룹이라 `editTm`이 갱신돼 있다.
            #   즉 «미설정»과 «비움»의 표현이 다를 뿐 **둘 다 0건**이다.
            #   초판은 이걸 `raise`로 받아 그 3그룹을 **매 스윕 영구 에러**로 만들었다(라이브
            #   실측 2026-08-17 13:02: detect 1회전 errors 3건이 전부 이것). 지금은 그 그룹에
            #   제외가 없어 손실이 없지만, 거기 제외가 생기는 순간 **영영 못 본다** —
            #   0건을 에러로 세는 것도 모름을 0건으로 세는 것만큼 나쁘다. 방향만 반대다.
            continue
        if not isinstance(items, list):
            # 리스트도 None도 아니면 그때는 스키마가 바뀐 것이므로 «0건»이라 말하지 않고
            # 예외로 올린다 — 모름을 0건으로 세는 것이 이 계열의 고질 결함이다(교훈 #123).
            raise WriteError(
