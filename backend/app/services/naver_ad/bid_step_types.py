...
# ── D-NAO-286: 손실 방어 표식 — **RL3 시간당 순위 고삐 전용** ──────────────────────
# ★이름을 정확히 쓴다(적대 리뷰 1R P1-1): 이 마커가 붙는 곳은 `auto_operator`의 `is_leash`
#   = **RL3 순위 고삐**(시간당 레인)다. 계약 §3이 이름으로 못 박은 **DL 일일 손실 고삐**는
#   일 레인의 무조건 승인 하향이라 마커가 아니라 **승인원**으로 면제한다
#   (`naver_execution_harness._FLOOR_EXEMPT_MARKER_SOURCES` 옆 주석 참조).
#   초판은 이 주석이 「DL 일일 손실 고삐」라 적혀 있어 **DL이 안 면제된 사실을 가렸다.**
