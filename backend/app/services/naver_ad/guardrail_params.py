...
#   존재 이유다.
# ★★**끄는 것은 «DB 층»뿐이다 — 「전부 코드 상수」가 아니다**(D-NAO-282 적대 리뷰 P1-1).
...
class ParamSpec:
    ...
    Attributes:
        ...
        default: 코드 상수(최후 폴백). guardrail_gate·runtime_switches에서 가져온다 — 두 곳에
            값을 적지 않는다.
        kind: 'decimal' | 'int' | 'bool'. ★'bool'은 D-NAO-282(계약 P2-ⓑ)이 킬스위치 2종을
        ...
        llm_proposable: 지혜 승격 판사(LLM)가 이 키의 변경을 «제안»할 수 있는가. 기본 True.
            ★D-NAO-282 적대 리뷰가 잡은 자리: `wisdom_judge`·`wisdom_apply`가 화이트리스트를
...
def get_switch(db: Session, key: str) -> bool:
    """bool 파라미터(킬스위치) 하나의 실효값. **폴백 규칙을 한 곳에 둔다** (D-NAO-282).
    ...
    try:
        ...
        # ★적대 리뷰 P2-2(D-NAO-282): DB 예외를 «삼키기만» 하면 세션이 rollback 대기 상태로
