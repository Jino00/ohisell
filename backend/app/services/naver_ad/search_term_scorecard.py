...
def build_scorecard(
    ...
    return {
        ...
        # ★2026-08-12 정정(D-NAO-177 적대 리뷰 P1-1): 종전 문구는 「실행 시점을 모르므로」였는데
        #   콘솔 「제외 검색어」 탭에 등록시각이 실제로 있어 이제 **아는 행이 섞인다**(같은 응답이
        #   dated>0을 낸다). 판정을 안 하는 진짜 이유는 시점 미상이 아니라 **사후 대조에 쓸 사전
        #   30일 검색어 데이터가 없다는 것**이다(가장 오래된 편입분은 2024-12-26이다).
        #   REVERT_HOWTO 탭 이름과 **같은 사실 갱신에서 나온 형제 문장**인데 한쪽만 고치면
        #   「한 파일 수리 ≠ 규율 채택」을 그대로 반복한다.
        "imported_unjudgeable_note": (
            "콘솔에 이미 걸려 있던 제외를 장부에 편입한 행이다. 등록시각을 아는 행도 있지만,"
            " 편입 시점 기준으로는 사후 대조에 쓸 사전 검색어 데이터가 없어 성적표가 판정하지"
            " 않는다 — 조치 생존 감시에는 포함된다."
