# slack_notifier.py — slack_notifier SA (P2-S3 T4, D-NAO-21)
        ...
        try:
            ...
            if resp.status_code == 200:
                return {
                    ...
                    "slack_ts": _extract_slack_ts(resp), "proposal_count": len(proposals),
                }
            last_err = f"HTTP {resp.status_code}: {resp.text[:200]}"
        if attempt < _MAX_RETRIES - 1:
            time.sleep(2 ** attempt)

    log.warning("Naver 제안 Slack 발송 실패(%d회 재시도): %s", _MAX_RETRIES, last_err)
    return {"sent": False, "reason": last_err, "slack_ts": None, "proposal_count": len(proposals)}
