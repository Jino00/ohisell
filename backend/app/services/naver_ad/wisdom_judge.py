...
#   중복 아닌가를 기준으로 verdict를 낸다. 회당 상한 5건(LLM 비용·시간). LLM 실패는 해당 후보
#   skip(fail-open — pending 유지, 내일 재시도). 읽기/쓰기 모두 wisdom_candidates 테이블만.
...
_SYSTEM = (
    ...
    "조건(요일계층·계절·아이폰 출시창)에서 같은 액션을 실행/차단했을 때 반복 관찰된 결과 패턴"
    "입니다. 다음 3가지를 기준으로 promote/reject를 판정하세요: "
    "①특정 날짜·단발 사건이 아니라 앞으로도 재사용 가능한 '판단원칙'인가, "
    "②환경조건↔결과의 연결이 이 후보의 데이터(반복 횟수·결과 방향)로 뒷받침되는가, "
...
def _prompt(cand: OpsWisdomCandidate, now: datetime) -> str:
    ...
    return (
        "아래는 지혜 승격 후보 1건입니다(JSON). 이 안의 정보만 근거로 판정하세요.\n\n"
