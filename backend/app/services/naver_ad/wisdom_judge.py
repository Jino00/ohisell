...
# ★D-NAO-282 — `SPECS.items()` 전건이 아니라 `llm_proposable_keys()`다. SPECS에 킬스위치가
...
_SYSTEM = (
    ...
    # ★D-NAO-282 적대 리뷰 2R P1 — 여기가 **넷째 자리**였다. 이 커밋이 「필터를 세 곳에 각자
    #   적으면 갈라진다」고 써 놓고 목록만 좁히고 **카운트를 안 고쳐** 「9종이라 말하고 7개를
    #   나열」했다. 실행 안전은 클램프가 지키지만(최악이 UNMAPPED 제안 1건), 판사에게는
    #   「2개가 더 있는데 안 보인다」는 신호라 **이름을 지어내라고 부추기는 모양**이 된다.
    #   ⇒ 카운트도 같은 출처에서 센다. 아래 회귀가 「카운트 == 나열 줄 수」를 못박는다.
    f"화이트리스트({len(guardrail_params.llm_proposable_keys())}종, 이 안에서만 고를 수 있습니다):\n{_PARAM_KEYS_DESC}\n"
    ...
    '손해를 보는가»**입니다. 위 화이트리스트 파라미터는 요일·계절·출시창·캠페인유형을 가리지 않고 모든 '
    ...
    "않습니다 — 전역 상수(위 화이트리스트)에 조건부 지혜를 반영하면 「주말에만 맞는 지혜가 "
