...
def import_drafts(
    ...
    * 둘 다    → 종전 동작 그대로.
    ...
    for (product_name, form_factor), rows in sorted(groups.items()):
        ...
        #   ⚠️**분할된 변형 레시피가 «원가만 재업로드» 경로(`_reimport_composition_only`)에서
        #     보호되는 것은 바로 이 규율에 «편승»한 결과다** (적대 리뷰 1R P2-2 채택).
        #     `grain_split.execute()`가 변형마다 `pick_cost_table_item`을 거쳐
        #     `picked_item_key`를 세우고 곧바로 승인까지 하므로, 저쪽 경로의 「승인·핀은
        #     안 건드린다」에 자동으로 걸린다. 즉 저기엔 **분할을 «알고» 짠 가드가 없다** —
        #     리뷰어가 우회 경로를 못 찾았지만(재현 실패 = 오늘은 안전), 저 두 규율 중
        #     하나라도 느슨해지면 분할이 조용히 풀린다. 그 의존을 여기 적어 둔다.
