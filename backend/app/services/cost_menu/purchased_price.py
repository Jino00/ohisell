    ...
    for sku in deduped:
        ...
        db.add(
            CostPurchasedPrice(
                ...
                approved_at=stamp,
                # ★★**여기서 문구를 «지어내지» 않는다.** 초판은 `note`가 비면
                #   "사람이 화면에서 매입품으로 분류하고 확정"을 박았는데, 프론트는 `note`를
                #   보내지도 않았고 API 직접 호출도 같은 문구를 받았다 — **이 층은 누가
                #   눌렀는지 알 수 없으면서 그것을 주장하고 있었다.** 완료 QA가 그 행을
                #   「클릭 주체 판정불능」으로 적으며 잡았다.
                #   ★이 저장소가 같은 모양을 이미 한 번 밟았다: n=16이 `cost_recipe.note`의
                #   「단가는 아직 없다」가 만들 때 한 번 박히고 아무도 안 고쳐 138행이
                #   거짓말하던 것을 백필로 고쳤다. 그때 배운 것은 **「기록은 아는 층이
                #   쓴다」**이지 「기본 문구를 잘 고르자」가 아니었다.
                #   ⇒ 호출자가 준 것만 싣는다. 화면에서 왔다는 사실은 **화면이** 말한다.
                note=note,
            )
        )
        result.written += 1
