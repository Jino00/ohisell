...
class TestReviveLane:
    def test_상한_값이_실제로_상한다(self):
        """★2R 생존 변이 M6: 상한 «기제»는 지켜지는데 상한 «값»은 아무도 안 봤다.

        아래 테스트들은 `_MAX_AUTO_REVIVES`를 인자로 되읽어 상대 비교만 하므로, 누가 그
        상수를 10**9로 바꿔도 전부 초록이다 — 그러면 「멈추는 자리」가 이름만 남는다.
        값 자체를 못박는다: 사람 개입이 필요한 상황에서 자동 재개를 열 번 넘게 시도하는 것은
        도움이 아니라 알림 폭주다.
        """
        assert 1 <= w._MAX_AUTO_REVIVES <= 10, (
            f"자동 재개 상한이 {w._MAX_AUTO_REVIVES} — 상한이 사실상 없는 값이면 "
            "P1-3의 2중 안전망이 이름만 남는다"
        )
